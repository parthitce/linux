...
extern void owl_wlan_bt_power(int on);

static int bt_powered_on;
static int acts_wlan_bt_ref_count;

static void owl_wlan_bt_power_control(int on)
{
	if (on) {
...
int bt_power_on(void)
{
	...
	owl_wlan_bt_power_control(1);
	msleep(50);

	check_and_set_gpio(bt_en_pin, 1);
	/*
...
void bt_power_off(void)
{
	...
	check_and_set_gpio(bt_en_pin, 0);

	owl_wlan_bt_power_control(0);
	msleep(50);
...
static int ubuntu_system_init_gpio(void)
{
	int ret;
...
static void ubuntu_system_deinit_gpio(void)
{
#ifdef HAVE_LPM
	gpio_free(bt_hwake_pin);
	gpio_free(bt_wake_pin);
