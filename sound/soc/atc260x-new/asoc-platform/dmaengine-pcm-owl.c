...
extern void set_dai_reg_base(int num);
...
#ifdef DUMP_DMA_DATA 
			if(bufferlen <= 512 * 1024 - snd_pcm_lib_period_bytes(substream)){
				if(buffer != NULL)memcpy(buffer + bufferlen,substream->dma_buffer.area + prtd->pos,snd_pcm_lib_period_bytes(substream));
...
int snd_dmaengine_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_START:
		...
		if(!strcasecmp(rtd->cpu_dai->name,"owl-audio-i2s") || !strcasecmp(rtd->cpu_dai->name,"owl-audio-i2s-6wire-tx")
		|| !strcasecmp(rtd->cpu_dai->name,"owl-audio-i2s-6wire-rx")){
			set_dai_reg_base(I2S_SPDIF_NUM);
			/* reset i2s rx&&tx fifo, avoid left & right channel wrong */
		    if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		    {
		        printk(KERN_ERR"reset and enable tx i2s fifo!\n");
		    ...
		    {
		        printk(KERN_ERR"reset and enable rx i2s fifo!\n");
				snd_dai_writel(snd_dai_readl(I2S_FIFOCTL)
				    & ~(0x3 << 9), I2S_FIFOCTL);
			    snd_dai_writel(snd_dai_readl(I2S_FIFOCTL)
				    | (0x3 << 9), I2S_FIFOCTL);
...
#ifdef DUMP_DMA_DATA
		if(buffer == NULL)
			buffer = kzalloc(512 * 1024, GFP_KERNEL);
#endif
...
#ifdef DUMP_DMA_DATA
		if(file == NULL)
            file = filp_open("/tmp/2.wav", O_WRONLY | O_CREAT, 0700);
	    if (IS_ERR(file)) {
		    printk(KERN_ERR"error occured while opening file /tmp/2.wav\n");
		}
		printk(KERN_ERR"Hello, I'm the module that intends to write messages to file.\n");

		loff_t pos = 0;
		mm_segment_t old_fs =get_fs();
    	set_fs(KERNEL_DS);
		if(buffer)
			ret = vfs_write(file, buffer, bufferlen, &pos);
		set_fs(old_fs);
		loff_t pos1 =file->f_pos;
		printk(KERN_ERR"[audio]pos1 is %lld,ret is %d\n",pos1,ret);
	...
	dma_filter_fn filter_fn, void *filter_data)
{
	...
	*/
	...
	return snd_dmaengine_pcm_open(substream, filter_data);
}
