	...
	struct snd_pcm_hw_params *params)
{
	...
	//set clk and cmu register when playback or this will cause noise
	//if(!strcasecmp(rtd->codec_dai->name,"NAU85L40-codec-dai")){
...
static struct snd_soc_dai_link s900_atc2603c_link_dai[] = {
	...
	{
		.name = "S900 nau85l40 AUDIO",
		.stream_name = "nau85l40 PCM",
		.cpu_dai_name = "owl-audio-spi",
		.codec_dai_name = "NAU85L40-codec-dai",
		.init = s900_link_snd_init,
		.platform_name = "s900-pcm-audio",
		.codec_name = "NAU85L40.1-001c",
		.ops = &s900_link_ops,
	},
};
...
static int __init s900_link_init(void)
{
	...
	if (earphone_gpio_num < 0) {
		...
		earphone_gpio_level = flags;//to do 
