...
//FIXME: for debug
/*
static u32 hdmihw_read_reg(u32 reg)
{
	//return readl(reg);
	return 0;
}
	 
static void hdmihw_write_reg(u32 val, u32 reg)
{
	//writel(val, reg);
}
*/
...
static int hdmi_EnableWriteRamPacket(void)
{
	int i;
	//set_hdmi_reg_base(0);
	hdmihw_write_reg(hdmihw_read_reg(HDMI_OPCR) | (0x1 << 31), HDMI_OPCR);
	while ((hdmihw_read_reg(HDMI_OPCR) & (0x1 << 31)) != 0) {
		for (i = 0; i < 10; i++)
			;
	}
	return 0;
...
static int hdmi_SetRamPacket(unsigned no, unsigned char *pkt)
{
	...
	/* write mode */
	//set_hdmi_reg_base(0);
	hdmihw_write_reg((1 << 8) | (((addr) & 0xFF) << 0), HDMI_OPCR);
	hdmihw_write_reg(reg[0], HDMI_ORP6PH);
	hdmihw_write_reg(reg[1], HDMI_ORSP6W0);
	hdmihw_write_reg(reg[2], HDMI_ORSP6W1);
	hdmihw_write_reg(reg[3], HDMI_ORSP6W2);
	hdmihw_write_reg(reg[4], HDMI_ORSP6W3);
	hdmihw_write_reg(reg[5], HDMI_ORSP6W4);
	hdmihw_write_reg(reg[6], HDMI_ORSP6W5);
	hdmihw_write_reg(reg[7], HDMI_ORSP6W6);
	hdmihw_write_reg(reg[8], HDMI_ORSP6W7);
...
static int hdmi_SetRamPacketPeriod(unsigned int no, int period)
{
	...
	/* disable */
	//set_hdmi_reg_base(0);
	hdmihw_write_reg(hdmihw_read_reg(HDMI_RPCR) & (unsigned int) (~(1 << no)),
	HDMI_RPCR);
	hdmihw_write_reg(hdmihw_read_reg(HDMI_RPCR) &
...
void set_hdmi_audio_interface(int channel, int samplerate)
{
	...
	hdmihw_write_reg(hdmihw_read_reg(HDMI_ICR) & ~(0x1 << 25), HDMI_ICR);
	hdmihw_write_reg(hdmihw_read_reg(HDMI_ACRPCR) | (0x1 << 31), HDMI_ACRPCR);
	hdmihw_read_reg(HDMI_ACRPCR); /*flush write buffer effect*/
	
	//set_hdmi_reg_base(0);
	tmp03 = hdmihw_read_reg(HDMI_AICHSTABYTE0TO3);
	tmp03 &= (~(0xf << 24));

	tmp47 = hdmihw_read_reg(HDMI_AICHSTABYTE4TO7);
	tmp47 &= (~(0xf << 4));
	tmp47 |= 0xb;
	...
	hdmihw_write_reg(tmp03, HDMI_AICHSTABYTE0TO3);
	hdmihw_write_reg(tmp47, HDMI_AICHSTABYTE4TO7);

	hdmihw_write_reg(0x0, HDMI_AICHSTABYTE8TO11);
	hdmihw_write_reg(0x0, HDMI_AICHSTABYTE12TO15);
	hdmihw_write_reg(0x0, HDMI_AICHSTABYTE16TO19);
	hdmihw_write_reg(0x0, HDMI_AICHSTABYTE20TO23);

	switch (channel) {
	case 2:
		hdmihw_write_reg(0x20001, HDMI_AICHSTASCN);
		break;

	case 3:
		hdmihw_write_reg(0x121, HDMI_AICHSTASCN);
		break;

	case 4:
		hdmihw_write_reg(0x2121, HDMI_AICHSTASCN);
		break;

	case 5:
		hdmihw_write_reg(0x12121, HDMI_AICHSTASCN);
		break;

	case 6:
		hdmihw_write_reg(0x212121, HDMI_AICHSTASCN);
		break;

	case 7:
		hdmihw_write_reg(0x1212121, HDMI_AICHSTASCN);
		break;

	case 8:
		hdmihw_write_reg(0x21212121, HDMI_AICHSTASCN);
		break;
	...
	/* 24 bit */
	hdmihw_write_reg((hdmihw_read_reg(HDMI_AICHSTABYTE4TO7) & ~0xf),
	HDMI_AICHSTABYTE4TO7);
	hdmihw_write_reg(hdmihw_read_reg(HDMI_AICHSTABYTE4TO7) | 0xb, HDMI_AICHSTABYTE4TO7);
	if (Audio60958 == 1) {
		switch (channel) {
		case 2:
			ASPCR = 0x00000011;
		...
		tmp03 |= 0x1;
		hdmihw_write_reg(tmp03, HDMI_AICHSTABYTE0TO3);
		hdmihw_write_reg(0x21, HDMI_AICHSTASCN);
	}

	/* enable Audio FIFO_FILL  disable wait cycle */
	hdmihw_write_reg(hdmihw_read_reg(HDMI_CR) | 0x50, HDMI_CR);

	if (samplerate > 7)
		ASPCR |= (0x1 << 31);

	hdmihw_write_reg(ASPCR, HDMI_ASPCR);
	hdmihw_write_reg(ACACR, HDMI_ACACR);
    /*非压缩格式23~30位写0
    * 如果针对压缩码流,
    ...
    * 如果针对线性PCM码流，则HDMI_AICHSTABYTE0TO3
    的bit[1:0]=0x0（同227A）;
    */
		hdmihw_write_reg(hdmihw_read_reg(HDMI_AICHSTABYTE0TO3) & ~0x3,
     ...
     */
		hdmihw_write_reg(hdmihw_read_reg(HDMI_ASPCR) & ~(0xff << 23), HDMI_ASPCR);

		hdmihw_write_reg(CRP_N | (0x1 << 31), HDMI_ACRPCR);
...
{
	printk(KERN_INFO"hdmi_audio_hw_params %d\n", audio_clk_enable);
...
static int __init s900_hdmi_init(void)
{
	...
	if (!s900_hdmi_device) {
		snd_err(
				"ASoC: Platform device s900-hdmi-audio allocation failed\n");
		ret = -ENOMEM;
		goto err;
	}
	...
	if (ret) {
		snd_err(
				"ASoC: Platform device s900-hdmi-audio add failed\n");
		goto err_device_add;
	}
	...
	if (ret) {
		snd_err(
				"ASoC: Platform driver s900-hdmi-audio register failed\n");
