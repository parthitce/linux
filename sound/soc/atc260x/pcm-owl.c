...
//			    DST_CONSTANT | SRC_DCU | DST_DEV | HDMIAUDIO | CRITICAL_BIT;
			dst_addr = I2S_SPDIF_BASE+SPDIF_DAT;
			break;
		case O_MODE_HDMI:
//			atslave->mode = PRIORITY_SEVEN | SRC_INCR |
//			    DST_CONSTANT | SRC_DCU | DST_DEV | HDMIAUDIO | CRITICAL_BIT;
			dst_addr =I2S_SPDIF_BASE+HDMI_DAT;
			break;
		case O_MODE_I2S:
		default:
//			atslave->mode = PRIORITY_SEVEN | SRC_INCR |
//			    DST_CONSTANT | SRC_DCU | DST_DEV | I2S_T | CRITICAL_BIT;
			//printk("%s %d\n", __FUNCTION__, __LINE__);
			dst_addr = I2S_SPDIF_BASE+I2STX_DAT;
			break;
		}

		//printk("%s %d, 0x%x\n", __FUNCTION__, __LINE__, dst_addr);		
...
//		    DST_INCR | DST_DCU | SRC_DEV | I2S_R | CRITICAL_BIT;
		...
		slave_config.src_addr = I2S_SPDIF_BASE+I2SRX_DAT;
...
//	snd_err("%s %d\n", __FUNCTION__, __LINE__);
	...
	//printk("%s %d: chan %d\n", __FUNCTION__, __LINE__, chan->chan_id);
	...
	int stream)
{
	...
	//printk("%s %d 0x%x, 0x%x, %d\n", __FUNCTION__, __LINE__, buf->addr, buf->area, buf->bytes);
	return 0;
}
