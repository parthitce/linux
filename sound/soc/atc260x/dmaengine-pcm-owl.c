...
extern struct dma_chan* dai_dma_slave_tx_chan();
extern struct dma_chan* dai_dma_slave_rx_chan();
extern struct dma_chan* dai_dma_slave_hdmia_chan();
	...
	struct dma_slave_config *slave_config)
{
	...
	//printk("%s %d: 0x%x\n", __FUNCTION__, __LINE__, slave_config->dst_addr);
}
...
static void dmaengine_pcm_dma_complete(void *arg)
{
	...
	//printk("%s %d period_len %d\n", __FUNCTION__, __LINE__, snd_pcm_lib_period_bytes(substream));
	...
	if(prtd->startflag == 1)
	{
		if (SNDRV_PCM_STREAM_CAPTURE == substream->stream )
		{
			...
			//printk("%s %d periods %d\n", __FUNCTION__, __LINE__, sum_periods);
			...
			//printk("%s %d remain %d, residue %d\n", __FUNCTION__, __LINE__, remain_frame_cnt, state.residue);
			//remain_frame_cnt = test_cnt-1;
			//test_cnt--;

			//int remain_frame_cnt = prtd->dma_chan->device->device_control(prtd->dma_chan, FSLDMA_EXTERNAL_START, 2);
			//printk("%d\n", remain_frame_cnt);
...
static int dmaengine_pcm_prepare_and_submit(struct snd_pcm_substream *substream)
{
	...
	//printk("%s %d src 0x%x\n", __FUNCTION__, __LINE__, substream->runtime->dma_addr);	
...
int snd_dmaengine_pcm_close_release_chan(struct snd_pcm_substream *substream)
{
	...
	//in 900A we use virtual channel, no need to release here
