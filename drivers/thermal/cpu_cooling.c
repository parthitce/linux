...
#ifdef CONFIG_OWL_CPU_AUTO_HOTPLUG
/* force shutdown cpus for thermal cooling */
extern int owl_hotplug_force_plugout(int enable);
#endif
...
#ifdef CONFIG_OWL_CPU_AUTO_HOTPLUG
		owl_hotplug_force_plugout(1);
#endif
		pr_info("[TMU] COOLING START, cooling_state:%ld\n", cooling_state);
	} else {
		event = CPUFREQ_COOLING_STOP;
#ifdef CONFIG_OWL_CPU_AUTO_HOTPLUG
		owl_hotplug_force_plugout(0);
#endif
		pr_info("[TMU] COOLING STOP, cooling_state:%ld\n", cooling_state);
	}
