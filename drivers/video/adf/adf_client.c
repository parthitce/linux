		...
		void *custom_data, size_t custom_data_size)
{
	...
	/*
	 * Actins code: wait if post_list is too large.
	 * */
	struct list_head *p;
	int post_list_count = 0;
	#define POST_LIST_MAX_NUM	(5)
	#define POST_WAIT_MS		(80)	/* 16ms(60Hz) * 5 = 80ms*/

	mutex_lock(&dev->post_lock);
	list_for_each(p, &dev->post_list) {
		++post_list_count;
	}
	mutex_unlock(&dev->post_lock);

	if (post_list_count > POST_LIST_MAX_NUM) {
		pr_info("ADF: post_list is too large(%d), wait %dms",
			post_list_count, POST_WAIT_MS);
		msleep(POST_WAIT_MS);
	}
	/*
	 * Actions code end.
