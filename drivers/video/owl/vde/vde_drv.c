...
static const struct of_device_id s900_vde_of_match[] = {
	{.compatible = "actions,s900-vde", .data = &s900_data},
	{}
};

MODULE_DEVICE_TABLE(of, s900_vde_of_match);
...
VDE_FREQ_T vde_do_set_freq(VDE_FREQ_T new_rate)
{
	...
	if (rate == targetfreq) {
		printk("vde : vde_do_set_freq: cur vde \
...
static VDE_FREQ_T vde_setfreq(VDE_FREQ_T freq_mhz)
{
	...
	*multi 704M  DISPLAY_PLL	 1.5  0.9v
	*4k 1056M    DISPLAY_PLL	 1    0.95v
	*/
...
static int asoc_vde_probe(struct platform_device *pdev)
{
	...
	id = of_match_device(s900_vde_of_match, &pdev->dev);
	if (id != NULL) {
	...
	VDE_CMU_COREPLL = devm_ioremap_nocache(&pdev->dev, CMU_BASE_PHY, 0x16);
	VDE_CMU_VDE_CLK = devm_ioremap_nocache(&pdev->dev, CMU_BASE_PHY + 0x40, 0x8);
	VDE_CMU_DEVRST0 = devm_ioremap_nocache(&pdev->dev, CMU_BASE_PHY + 0xa8, 0x8);
	VDE_CMU_DEVCLKEN0 = devm_ioremap_nocache(&pdev->dev, CMU_BASE_PHY + 0xa0, 0x8);
...
static struct platform_driver vde_platform_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
		.of_match_table = s900_vde_of_match,
	},
	.suspend = vde_suspend,
	.resume = vde_resume,
