...
/* those registers related with VCE */
/* reset */
#define CMU_DEVRST0             (0xE0160000 + 0xA8)
...
#define CMU_DEVCLKEN0           (0xE0160000 + 0xA0)
#define CMU_DEVPLL              (0xE0160000 + 0x04)
#define CMU_DDRPLL              (0xE0160000 + 0x08)
#define CMU_DISPLAYPLL          (0xE0160000 + 0x10)
#define CMU_VCECLK              (0xE0160000 + 0x44)
...
long vce_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if ((read_reg_self(iobase_cmu_devclken0) & VCE_CLKEN) == 0) {
		vce_err("vce clk is not enabled,CMU_DEVCLKEN0 = %x\n",
			read_reg_self(iobase_cmu_devclken0));
		return VCE_ERR_UNKOWN;
	}

	if ((read_reg_self(iobase_sps_pg_ctl) & VCE_BISP_POWERON) == 0) {
	...
	if (res != NULL) {
		if (request_mem_region(res->start, resource_size(res), "vce") !=
			...
			iobase_cmu_devclken0 =
			    (unsigned long)ioremap(CMU_DEVCLKEN0, 4);
			...
			iobase_sps_pg_ctl =
			    (unsigned long)ioremap(SPS_PG_CTL, 4);
