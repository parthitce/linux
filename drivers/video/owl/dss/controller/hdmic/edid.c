...
int hdmi_edid_parse(struct hdmi_edid *edid)
{
	...
	edid->video_formats[3] |= 0x80000000;	/* 127, 3840x1080@60 */
#endif
#ifdef HDMI_VID_2560_1024_75
	edid->video_formats[3] |= 0x40000000;	/* 126, 2560x1024@75 */
#endif
#ifdef HDMI_VID_2560_1024_60
	edid->video_formats[3] |= 0x20000000;	/* 125, 2560x1024@60 */
#endif
#ifdef HDMI_VID_1280_1024_60
	edid->video_formats[3] |= 0x10000000;	/* 124, 1280x1024@60 */
#endif
		...
		if (offset > 4) {
			...
			while (bsum < offset) {
				...
				if (len > 0 && (bsum + len + 1) > offset) {
					pr_debug("len or bsum size error\n");
					continue;
				}
...
err1:
	for (i = 0; i < 128; i++)
		edid->video_formats[i / 32] |= (edid->device_support_vic[i]
						<< (i % 32));

	edid->video_formats[0] |= 0x04;

	edid->read_ok = 1;
	pr_debug("edid->video_formats[0] = 0x%x\n", edid->video_formats[0]);
	pr_debug("edid->video_formats[1] = 0x%x\n", edid->video_formats[1]);
	pr_debug("edid->video_formats[2] = 0x%x\n", edid->video_formats[2]);
	pr_debug("edid->video_formats[3] = 0x%x\n", edid->video_formats[3]);
