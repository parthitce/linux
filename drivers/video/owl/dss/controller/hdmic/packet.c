...
int hdmi_gen_avi_infoframe(struct hdmi_ip *ip)
{
	...
	if (true &&		/* DisplayFormat == 0, TODO */
	    (ip->cfg->vid == VID3840x2160p_24 ||
	     ip->cfg->vid == VID3840x2160p_25 ||
	     ip->cfg->vid == VID3840x2160p_30 ||
	     ip->cfg->vid == VID4096x2160p_24))
		pkt[7] = 0;
	else
		pkt[7] = ip->cfg->vid;
...
int hdmi_gen_vs_infoframe(struct hdmi_ip *ip)
{
	 ...
	 */
	if (ip->cfg->vid == VID3840x2160p_24 ||
	    ip->cfg->vid == VID3840x2160p_25 ||
	    ip->cfg->vid == VID3840x2160p_30 ||
	    ip->cfg->vid == VID4096x2160p_24) {
		switch (ip->cfg->vid) {
	...
	} else if (ip->settings.mode_3d != MODE_3D_DISABLE) {
		...
		case MODE_3D_LR_HALF:
			pkt[8] = 0x8 << 4;
			pkt[9] = 0x1 << 4;
			break;

		case MODE_3D_TB_HALF:
			pkt[8] = 0x6 << 4;
			pkt[9] = 0x0;
			break;
