...
static char default_hdmi_size[64] = "100x100";
module_param_string(default_hdmi_size, default_hdmi_size, 64, 0644);


/* FIXME */
static void __iomem *ip_base_to_sound;
...
static void hdmic_update_mode_list(struct hdmi_data *hdmi)
{
	...
	/*
	 * if hdmi is not powered on in u-boot,
	 ...
	 */
	if (ip->ops->is_power_on && !ip->ops->is_power_on(ip)) {
		if (n_modes > 0) {
...
{
	static bool dummy_plugout_sent = false;
	...
	if (!status && !hdmi->cable_status) {
		if (dummy_plugout_sent)
			return;
		else
			dummy_plugout_sent = true;
	}
...
static int __init default_hdmi_size_setup(char *str)
{
	strcpy(default_hdmi_size, str);

	return 1;
}
__setup("actions.hdmi.size=", default_hdmi_size_setup);
			       ...
			       struct owl_panel *panel)
{
	...
	sscanf(default_hdmi_size, "%dx%d", &x, &y);
	owl_panel_set_scale_factor(panel, x, y);
...
static int owl_hdmic_enable(struct owl_display_ctrl *ctrl)
{
	...
	/* update cfg before enabling */
	hdmic_update_cfg(hdmi);

	if (ip_ops->power_on && ip_ops->power_on(ip) < 0)
		return -EINVAL;
...
static int owl_hdmic_disable(struct owl_display_ctrl *ctrl)
{
	...
	if ((hdmi->property.hdcp_onoff || test_hdcp_onoff == 1) &&
	     ip->ops->hdcp_enable)
		ip->ops->hdcp_enable(ip, false);

	/* update cfg before disabling */
	hdmic_update_cfg(hdmi);
...
static void owl_hdmic_hpd_enable(struct owl_display_ctrl *ctrl, bool enable)
{
	...
	} else if (!PANEL_HOTPLUG_ALWAYS_ON(ctrl->panel) &&
		   !enable && hdmi->hpd_en) {
		hdmi->hpd_en = false;
...
static int owl_hdmic_mode_3d_set(struct owl_display_ctrl *ctrl,
				 enum owl_3d_mode mode_3d)
{
	struct hdmi_data *hdmi = owl_ctrl_get_drvdata(ctrl);

	dev_dbg(hdmi->dev, "%s, mode_3d %d\n", __func__, mode_3d);

	if (mode_3d == hdmi->ip->settings.mode_3d)
		return 0;

	hdmi->ip->settings.mode_3d = mode_3d;

	/*
	 * off and on to take effect.
	 *
	 * if it is frame 3D, we have to change HDMI PLL,
	 * so power off/power on should be used instead.
	 * TODO FIXME
	 */
	hdmi->ip->ops->video_disable(hdmi->ip);
	msleep(500);
	hdmi->ip->ops->video_enable(hdmi->ip);

	return 0;
}

static enum owl_3d_mode owl_hdmic_mode_3d_get(struct owl_display_ctrl *ctrl)
{
	struct hdmi_data *hdmi = owl_ctrl_get_drvdata(ctrl);
...
static struct owl_display_ctrl_ops owl_hdmi_ctrl_ops = {
	...
	.mode_3d_set = owl_hdmic_mode_3d_set,
	.mode_3d_get = owl_hdmic_mode_3d_get,
};
...
static int hdmi_ip_init(struct hdmi_ip *ip)
{
	...
	settings->mode_3d = MODE_3D_DISABLE;
