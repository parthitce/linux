...
static ssize_t show_dsi_dump(struct device *dev,
...
static ssize_t test_dsi_fs(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	test_fs_dsi(dev);
	return 0;
...
static ssize_t test_fs_dsi_cmd(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	test_fs_longcmd(dev);
	return 0;
...
static struct device_attribute owl_dsi_attrs[] = {
...
int owl_dsi_create_sysfs(struct device *dev)
{
	...
	for (t = 0; t < ARRAY_SIZE(owl_dsi_attrs); t++) {
		r = device_create_file(dev, &owl_dsi_attrs[t]);
...
void owl_dsi_remove_sysfs(struct device *dev)
{
	...
	for (t = 0; t < ARRAY_SIZE(owl_dsi_attrs); t++)
		device_remove_file(dev, &owl_dsi_attrs[t]);
}
