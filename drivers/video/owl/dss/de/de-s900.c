				    ...
				    enum owl_display_type type)
{
	...
	if (type != OWL_DISPLAY_TYPE_HDMI) {
		...
		case OWL_DISPLAY_TYPE_LCD:
			...
			path->preline_en_mask = (1 << 1);
			path->preline_pending_mask = (1 << 1);
			path->vb_active_mask = (1 << 9);
			break;
		case OWL_DISPLAY_TYPE_DSI:
			...
			path->preline_en_mask = (1 << 2);
			path->preline_pending_mask = (1 << 2);
			path->vb_active_mask = (1 << 10);
			break;
		case OWL_DISPLAY_TYPE_EDP:
			...
			path->preline_en_mask = (1 << 3);
			path->preline_pending_mask = (1 << 3);
			path->vb_active_mask = (1 << 11);
			break;
		default:
			BUG();
			break;
...
{
	return !!(de_readl(DE_IRQENABLE) & path->preline_en_mask);
}

static bool de_s900_path_is_preline_pending(struct owl_de_path *path)
{
	return !!(de_readl(DE_IRQSTATUS) & path->preline_pending_mask);
}

static void de_s900_path_clear_preline_pending(struct owl_de_path *path)
{
	de_writel(path->preline_pending_mask, DE_IRQSTATUS);
}

static bool de_s900_path_is_vb_valid(struct owl_de_path *path)
{
	return !!(de_readl(DE_IRQSTATUS) & path->vb_active_mask);
}
...
static struct owl_de_path de_s900_paths[] = {
	{
		...
		/* path0 is fixed for HDMI */
		.preline_en_mask	= (1 << 0),
		.preline_pending_mask	= (1 << 0),
		.vb_active_mask		= (1 << 8),
	},
	{
		.id			= 1,
		...
		/*
		 * default for LCD, should be re-set at
		 * '__path_display_type_set'
		 */
		.preline_en_mask	= (1 << 1),
		.preline_pending_mask	= (1 << 1),
		.vb_active_mask		= (1 << 9),
	},
};
...
static void __video_alpha_set(struct owl_de_path *path,
			      struct owl_de_video *video,
			      uint8_t alpha_value, bool alpha_en,
			      bool pre_mult_en)
{
	uint32_t val;

	de_debug("%s, global_alpha %d, global_alpha_en %d, pre_mult_en %d\n",
		 __func__, alpha_value, alpha_en, pre_mult_en);
...
static void de_s900_video_apply_info(struct owl_de_video *video)
{
	...
	__video_alpha_set(video->path, video, info->global_alpha,
			  info->global_alpha_en, info->pre_mult_alpha_en);
}
