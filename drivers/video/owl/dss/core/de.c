...
static int				skip_3d_set;

/*
 * a lock to resolve the sync problem between
 * wait thread(UI) and suspend thread(power manage)
 * TODO, FIXME
 */
static DEFINE_MUTEX(wait_suspend_lock);
...
int owl_de_generic_suspend(struct device *dev)
{
	...
	/* block owl_de_path_wait_for_go */
	mutex_lock(&wait_suspend_lock);
...
int owl_de_generic_resume(struct device *dev)
{
	...
	/* release lock, owl_de_path_wait_for_go can continue */
	mutex_unlock(&wait_suspend_lock);
...
int owl_de_path_enable_vsync(struct owl_de_path *path)
{
	...
	if (atomic_inc_return(&path->vsync_enable_cnt) == 1)
...
int owl_de_path_disable_vsync(struct owl_de_path *path)
{
	...
	if (atomic_dec_and_test(&path->vsync_enable_cnt))
...
/*
 * check if need set this path's 3D mode,
 * for simple, set 3d mode according to one of the video layer's 3D mode.
 * if there are different 3d mode in one path, just choose one.
 */
static void de_path_mode_3d_set(struct owl_de_path *path)
{
	enum owl_3d_mode mode_3d = MODE_3D_DISABLE;
	struct owl_de_video *video;
	...
	list_for_each_entry(video, &path->videos, list) {
		if (video->info.mode_3d != MODE_3D_DISABLE) {
			mode_3d = video->info.mode_3d;
			break;
		}
	}
	...
	owl_panel_3d_mode_set(path->panel, mode_3d);
}

void owl_de_path_apply(struct owl_de_path *path)
{
	bool need_set_go = false;
	struct owl_de_video *video;
...
void owl_de_path_wait_for_go(struct owl_de_path *path)
{
	...
	/* if suspended, block the wait thread till resuming */
	mutex_lock(&wait_suspend_lock);

	/* FCR is zero, return directly */
	...
	/* HDMI, TODO, FIXME */
	ret = wait_event_interruptible_timeout(path->vsync_wq,
	...
	ret = WAIT_WITH_TIMEOUT(!path->ops->is_fcr_set(path), 10);
	if (ret == 0)
...
EXPORT_SYMBOL(owl_de_path_clear);
			       ...
			       struct owl_de_video_info *info)
{
	...
	if (info->pre_mult_alpha_en && info->global_alpha != 255)
		ERR_OUT(-4);
 ...
 */
#define OWL_DE_GAMMA_ENABLEX

/*
 * wait VB in IRQ or workqueue.
 *
 * NOTE:
 *	If 'OWL_DE_WAIT_VB_IN_WORK' is defined,
 *	VB will be waited in workqueue,
 *	due to the large schedule latency,
 *	we need a large preline time, such as 300us.
 *
 *	If 'OWL_DE_WAIT_VB_IN_WORK' is undefined,
 *	VB will be waited in IRQ, to avoid large irq latency,
 *	we need a small preline time, such as 60us.
 *
 *	So, if you change this config, please also re-check
 *	'DSS_RECOMMENDED_PRELINE_TIME', which is defined in
 *	'include/video/owldss.h' and 'owl/platform/boot/lib/dss/dss.h'
 */
#define OWL_DE_WAIT_VB_IN_WORKX
...
#ifdef OWL_DE_WAIT_VB_IN_WORK
	if (!__de_path_wait_vb_valid(path))
		return;
#endif

	/* gamma & histogram, TODO */
...
static irqreturn_t de_irq_handler(int irq, void *data)
{
	...
	for (i = 0; i < de->num_paths; i++) {
		...
		if (path->ops->is_preline_enabled(path) &&
			...
			/* flush posted write */
			path->ops->is_preline_enabled(path);

			/* handling ... */
			path->vsync_stamp = ktime_get();

			#ifndef OWL_DE_WAIT_VB_IN_WORK
				if (__de_path_wait_vb_valid(path))
			#endif
...
static void de_video_debugfs_create(struct owl_de_video *video)
{
	...
	debugfs_create_bool("global_alpha_en", S_IRUGO | S_IWUSR, video->dir,
			    (u32 *)&info->global_alpha_en);
	debugfs_create_bool("pre_mult_alpha_en", S_IRUGO | S_IWUSR, video->dir,
			    (u32 *)&info->pre_mult_alpha_en);
	debugfs_create_u8("global_alpha", S_IRUGO | S_IWUSR, video->dir,
			  &info->global_alpha);
...
module_param(skip_3d_set, int, 0644);
