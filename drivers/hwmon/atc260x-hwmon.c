			  ...
			  struct device_attribute *attr, char *buf)
{
	...
	if (channel < 0) {
		...
		uint i;
		for (i = 0; i < ARRAY_SIZE(sc_atc260x_hwmon_alt_ch_name_tbl);
		     i++) {
			if (strcmp
			    (sen_attr->dev_attr.attr.name,
			     sc_atc260x_hwmon_alt_ch_name_tbl[i][1]) == 0) {
				core_ch_name =
				    sc_atc260x_hwmon_alt_ch_name_tbl[i][0];
				channel =
				    atc260x_auxadc_find_chan(hwmon->atc260x,
							     core_ch_name);
	...
	if (ret != 0) {
		...
		ch_unit_name =
		    atc260x_auxadc_channel_unit_name(hwmon->atc260x, channel);
...
static int atc260x_hwmon_probe(struct platform_device *pdev)
{
	...
	hwmon =
	    devm_kzalloc(&pdev->dev, sizeof(struct atc260x_hwmon_dev),
			 GFP_KERNEL);
	if (!hwmon)
		return -ENOMEM;
