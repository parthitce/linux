...
static unsigned int gyro_irq;
unsigned int gyro_irq_gpio;
...
{
	gyro_regulator = regulator_get(NULL, regulator_cfg.regulator);
	pr_info("[%s]regulator init\n", __func__);
	if (IS_ERR(gyro_regulator)) {
		pr_err("Nova err,regulator_get fail\n!!!");
		return NULL;
	}

	if (regulator_set_voltage(gyro_regulator, minvol, maxvol)) {
		pr_err("Nova err,cannot set voltage\n!!!");
		regulator_put(gyro_regulator);
		return NULL;
	}
	regulator_enable(gyro_regulator);
	return gyro_regulator;
}
...
int inv_reset_offset_reg(struct inv_mpu_state *st, bool en)
{
	...
	if (en) {
		for (i = 0; i < 3; i++) {
			...
			st->input_gyro_offset[i] = st->rom_gyro_offset[i];
			st->input_accel_offset[i] = st->rom_accel_offset[i];
		}
	} else {
		for (i = 0; i < 3; i++) {
			gyro[i] = st->rom_gyro_offset[i] +
		...
		const struct i2c_device_id *id)
{
	...
	if (!strcmp(id->name, "itg3500")) {
		...
		st->chip_type = INV_MPU6500;
		printk("-----st->chip_type = INV_MPU6500-----\n");
	...
	case INV_MPU6500:
		if (SECONDARY_SLAVE_TYPE_COMPASS == plat->sec_slave_type) {
			conf->has_compass = 1;
			printk("conf->has_compass = 1\n");
		} else {
			conf->has_compass = 0;
			printk("conf->has_compass = 0\n");
		}
	...
	if (result) {
		pr_err("read silicon rev error,result:%d\n", result);
...
static int inv_mpu_work_func()/*(struct work_struct *work)*/
{
...
static int inv_mpu_resume(struct device *dev)
{
	...
	if (!IS_ERR(gyro_regulator)) {
		regulator_set_voltage(gyro_regulator,
			regulator_cfg.vol_min, regulator_cfg.vol_max);
		ret = regulator_enable(gyro_regulator);
	}
...
static int inv_mpu_suspend(struct device *dev)
{
	...
	if (st->chip_config.dmp_on && st->chip_config.enable) {
		...
		/* reload firmware after suspend */
		st->chip_config.firmware_loaded = 0;

	mutex_unlock(&indio_dev->mlock);

	if (!IS_ERR(gyro_regulator))
		regulator_disable(gyro_regulator);
	return result;
}
...
static int regulator_cfg_get(struct device_node *of_node)
{
	...
	/* regulator */
	if (of_find_property(of_node, "gyro_vcc", NULL)) {
		ret = of_property_read_string(of_node,
			"gyro_vcc", &regulator_cfg.regulator);
		if (ret < 0) {
			pr_err("can not read gyro_vcc power source\n");
			return -1;
		} 
		...
		regulator_cfg.vol_min = scope[0];
		regulator_cfg.vol_max = scope[1];
	}
	return 0;
}
static int inv_of_data_get(void)
{
	...
	regulator_cfg_get(of_node);
	
	ret = of_property_read_u32(of_node, CFG_GYROSENSOR_ADAP_ID, &cfg_i2c_adap_id);
	if (ret != 0) {
		printk(KERN_ERR"get i2c_adapter_id %d fail\n", cfg_i2c_adap_id);
		cfg_i2c_adap_id = 1;
	}

	gyro_irq = irq_of_parse_and_map(of_node, 0);
	printk("irq number=%d\n", gyro_irq);

	gyro_irq_gpio = of_get_gpio(of_node, 0);
	printk("irq gpio number=%d\n", gyro_irq_gpio);
	...
	if (of_property_read_string(of_node, CFG_GYROSENSOR_MASTER_INV6515_POSITION, &buf)) {
		...
		strict_strtol(buf, 10, &master_position);
		printk("gsensor dt read INV6515 position is %d\n", master_position);
		...
		case -4:
			...
			break;
		default:
			printk(" invalid master position: %d \n", master_position);
		  break;
		}
	}
	...
	printk(KERN_ERR"%s,%d, failed to read ak8963c position!\n", __func__, __LINE__);
		...
		strict_strtol(buf, 10, &secondary_position);
		printk("gsensor dt read ak8963c position is %d\n", secondary_position);
		...
		case -4:
			...
			break;
		default:
			printk(" invalid secondary_position: %d \n", secondary_position);
		  break;
		}
	}
	...
	if (of_property_read_string(of_node, CFG_GYROSENSOR_SECONDARY_I2C_ADDR, &buf)) {
		...
		strict_strtol(buf, 10, &secondary_i2cAddr);
...
static int __init inv_mpu_init(void)
{
	...
	if (result < 0)
		return -1;
	/* regulator init */
	printk("regulator begin...\n");
	regulator_init(regulator_cfg.regulator,
	   regulator_cfg.vol_min, regulator_cfg.vol_max);

	i2c_adap = i2c_get_adapter(cfg_i2c_adap_id);
	single_chip_board_info->irq = gyro_irq;
	printk(" single_chip_board_info.irq = %d\n", single_chip_board_info->irq);
