...
#define CFG_GYROSENSOR_MASTER_INV6500_POSITION     "MASTER_inv6500_position"
...
static int inv_set_offset_reg(struct inv_mpu_iio_s *st, int reg, int val)
{
	int result;
	u8 d;
...
int inv_reset_offset_reg(struct inv_mpu_iio_s *st, bool en)
{
	const u8 *ch;
	int i, result;
	s16 gyro[3], accel[3];
	...
	struct device_attribute *attr, char *buf)
{
	...
	case ATTR_ACCEL_SCALE:
	{
		const s16 accel_scale[] = {2, 4, 8, 16};
		return sprintf(buf, "%d\n",
					accel_scale[st->chip_config.accl_fs] *
					st->chip_info.multi);
	}
	case ATTR_COMPASS_SCALE:
	{
		//st->slave_compass->get_scale(st, &result);
        result = DATA_AKM8963_SCALE1;
		return sprintf(buf, "%d\n", result);	
	}
...
static int inv_of_data_get(void)
{
	...
	if (of_property_read_string(of_node, CFG_GYROSENSOR_MASTER_INV6500_POSITION, &buf)) {
