...
int inv_mpu_configure_ring(struct iio_dev *indio_dev)
{
	...
	extern unsigned int gyro_irq_gpio;
	printk("inv_mpu_configure_ring\n");
	...
	if (INV_MPU3050 == st->chip_type) {
		...
		printk("request_threaded_irq:%d\n", st->client->irq);
		 ...
		 if (gpio_request(gyro_irq_gpio, "inv_irq") != 0) {
			printk("%s:gpio_irq gpio_request() error\n", __FUNCTION__);
			return -EIO;
		} else {
			printk("request irq  gpio success\n");
		}
		gpio_direction_input(gyro_irq_gpio);
