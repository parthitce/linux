...
void aotg_udc_dump_td(struct aotg_ep *ep)
{
	...
	pr_debug("===========ring infos==============:\n");
	pr_debug("first_trb:%p, last_trb:%p,enqueue_trb:%p, cur_trb:%p, trb_dma:%p, num_trbs_free:%d\n",
...
void aotg_dump_ring(void)
{
	...
	for (i = 0; i < AOTG_UDC_NUM_ENDPOINTS; i++) {
		...
		if (!ring)
			continue;
		pr_debug("===========ring infos==============:\n");
		pr_debug("first_trb:%p, last_trb:%p, enqueue_trb:%p, cur_trb:%p, trb_dma:%d, num_trbs_free:%d\n",
				ring->first_trb, ring->last_trb, ring->enqueue_trb, ring->cur_trb, ring->trb_dma, ring->num_trbs_free);
		trb = ring->first_trb;
		for (trb = ring->first_trb; trb < ring->last_trb; trb++) {
			pr_debug("hw_buf_ptr:%x, hw_buf_len:%x, hw_buf_remain:%x, hw_token:%x\n",
...
void aotg_udc_dma_handler(struct aotg_udc *udc, u8 dma_number)
{
	...
	if (i == AOTG_UDC_NUM_ENDPOINTS) {
		pr_err("can't find correct ep no in dma irq! dma_number:%d\n", dma_number);
		return;
	}
...
void done(struct aotg_ep *ep, struct aotg_request *req, int status)
{
	...
	if (likely(req->req.status == -EINPROGRESS)) {
		...
		pr_debug("status:%x req.status:%x\n", status, req->req.status);
		status = req->req.status;
	}
...
void aotg_udc_endpoint_config(struct aotg_udc *udc)
{
	...
	UDC_BULK_EP(8, "ep4out", USB_DIR_OUT | 4, EPCON_BUF_SINGLE);
	UDC_BULK_EP(9, "ep5in", USB_DIR_IN | 5, EPCON_BUF_SINGLE);
	UDC_BULK_EP(10, "ep5out", USB_DIR_OUT | 5, EPCON_BUF_SINGLE);
