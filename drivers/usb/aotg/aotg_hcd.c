	...
	unsigned mem_flags)
{
	...
	if (likely(urb->ep->hcpriv)) {
		...
		if ((type == PIPE_BULK) && (ic_type[acthcd->id] == S900))
...
static int aotg_hcd_hub_resume(struct device *dev)
{
	...
	if (aotg_uhost_mon[pdev->id])
		aotg_uhost_mon[pdev->id]->det_step = 0;
	...
	printk(KERN_INFO"aotg_hcd.%d ==> %s\n", pdev->id, __func__);
	aotg_power_onoff(acthcd->id, 1);
	mdelay(1);
	aotg_powergate_on(acthcd->id);
