...
unsigned int port_host_plug_detect[2] = {0};
unsigned int aotg_wake_lock[2] = {0};
int vbus_otg_en_gpio[2][2];
static int aotg_initialized[2] = {0};
/*if detect low speed device plug in,must disable usbh high speed*/
int is_ls_device[2] = {0};
...
{
	if (0 == dev_id) {
		aotg_data[dev_id].usbecs = devm_ioremap_nocache(dev, USBH_0ECS, 4);
		aotg_data[dev_id].usbpll = devm_ioremap_nocache(dev, CMU_USBPLL, 4);
		aotg_data[dev_id].devrst = devm_ioremap_nocache(dev, CMU_DEVRST1, 4);
		aotg_data[dev_id].sps_pg_ctl = devm_ioremap_nocache(dev, SPS_PG_CTL, 4);
		aotg_data[dev_id].sps_pg_ack = devm_ioremap_nocache(dev, SPS_PG_ACK, 4);

		aotg_data[dev_id].usbpll_bits = CMU_USB0PLL_USBPLLEN;
		aotg_data[dev_id].devrst_bits = CMU_DEVRST1_USBH0;
		aotg_data[dev_id].pg_ctl_bits = USB0_PG_CTL_BITS;
		aotg_data[dev_id].pg_ack_bits = USB0_PG_ACK_BITS;
		aotg_data[dev_id].no_hs = 0;
	} else if (1 == dev_id) {
		aotg_data[dev_id].usbecs = devm_ioremap_nocache(dev, USBH_1ECS, 4);
		aotg_data[dev_id].usbpll = devm_ioremap_nocache(dev, CMU_USBPLL, 4);
		aotg_data[dev_id].devrst = devm_ioremap_nocache(dev, CMU_DEVRST1, 4);
		aotg_data[dev_id].sps_pg_ctl = devm_ioremap_nocache(dev, SPS_PG_CTL, 4);
		aotg_data[dev_id].sps_pg_ack = devm_ioremap_nocache(dev, SPS_PG_ACK, 4);

		aotg_data[dev_id].usbpll_bits = CMU_USB1PLL_USBPLLEN;
		aotg_data[dev_id].devrst_bits = CMU_DEVRST1_USBH1;
		aotg_data[dev_id].pg_ctl_bits = USB1_PG_CTL_BITS;
		aotg_data[dev_id].pg_ack_bits = USB1_PG_ACK_BITS;
		aotg_data[dev_id].no_hs = 0;
	} else {
		BUG_ON(1);
	}
...
void aotg_power_onoff(int id, int on_off)
{
	...
	if (port_host_plug_detect[id] == 3) {
		/*if the other port is working;don't change vbus status*/
		if (act_hcd_ptr[1-id] != NULL)
			return;
	}
...
static int aotg_hcep_config_iso(struct aotg_hcd *acthcd,
	struct aotg_hcep *ep, u8 type, u8 buftype, int is_out)
{
	int index = 0;
	ulong addr = 0;
	int get_ep = 0;
	...
	int ep_num, int is_in)
{
	...
	pr_warn("hcd ep err ep_num:%d, is_in:%d\n", ep_num, is_in);
	...
	default:
		pr_warn("err_val:0x%x, err_type:%d\n", err_val, err_type);
		if (is_in) {
			pr_warn("HCINEP%dSPILITCS:0x%x\n", ep_num,
				readb(acthcd->base + ep_num * 8 + HCEP0SPILITCS));
		} else {
			pr_warn("HCOUTEP%dSPILITCS:0x%x\n", ep_num,
...
static void port_suspend(struct aotg_hcd *acthcd)
{
	...
	usb_setbitsb(SUSPEND_IRQIEN, acthcd->base + USBEIEN);
	usb_clearbitsb(OTGCTRL_BUSREQ, acthcd->base + OTGCTRL);
}
...
static irqreturn_t aotg_hub_irq(struct usb_hcd *hcd)
{
	...
	pr_info("USBEIEN:0x%x, USBEIRQ:0x%x, ivec:0x%x\n", readb(acthcd->base + USBEIEN),
		readb(acthcd->base + USBEIRQ), irqvector);
	pr_info("HCINxDMAIRQ0:0x%x, HCOUTxDMAIRQ0:0x%x\n",
	...
	if (eirq_pending & SUSPEND_IRQIEN) {
...
static enum hrtimer_restart aotg_hub_hotplug_timer(struct hrtimer *hrtimer)
{
	...
	if (unlikely(IS_ERR_OR_NULL((void *)acthcd))) {
		ACT_HCD_DBG
...
static int aotg_hub_suspend(struct usb_hcd *hcd)
{
	...
	for (i = 1; i < MAX_EP_NUM; i++) {
		if (acthcd->outep[i] != NULL) {
			aotg_stop_ring(acthcd->outep[i]->ring);
			ep_disable(acthcd->outep[i]);
		}

		if (acthcd->inep[i] != NULL) {
			aotg_stop_ring(acthcd->inep[i]->ring);
			ep_disable(acthcd->inep[i]);
		}
	}
	if (acthcd->inserted)
		acthcd->bus_remote_wakeup = 1;
...
static int aotg_hub_resume(struct usb_hcd *hcd)
{
	struct aotg_hcd *acthcd = hcd_to_aotg(hcd);
...
{
	aotg_DD_set_phy(acthcd->base, 0xe7, 0x1b);
	aotg_DD_set_phy(acthcd->base, 0xe7, 0x1f);
	udelay(10);
	aotg_DD_set_phy(acthcd->base, 0xe0, 0xa3);
	aotg_DD_set_phy(acthcd->base, 0x87, 0x1f);
	aotg_DD_set_phy(acthcd->base, 0xe3, 0x0b);

	return;
}


int aotg_hcd_wait_reset(struct aotg_hcd *acthcd)
{
	int i = 0;
...
void aotg_hcd_powergate_on(struct aotg_hcd *acthcd)
{
	struct device *dev = acthcd->dev;
	/*struct aotg_plat_data *data = acthcd->port_specific;*/
	pm_runtime_enable(dev);
	pm_runtime_get_sync(dev);

	clk_prepare_enable(aotg_data[acthcd->id].clk_usbh_phy);
	clk_prepare_enable(aotg_data[acthcd->id].clk_usbh_cce);
	clk_prepare_enable(aotg_data[acthcd->id].clk_usbh_pllen);
}

void aotg_hcd_powergate_off(struct aotg_hcd *acthcd)
{
	struct device *dev = acthcd->dev;

	clk_disable_unprepare(aotg_data[acthcd->id].clk_usbh_pllen);
	clk_disable_unprepare(aotg_data[acthcd->id].clk_usbh_phy);
	clk_disable_unprepare(aotg_data[acthcd->id].clk_usbh_cce);
...
static void aotg_hcd_hardware_init(struct aotg_hcd *acthcd)
{
	...
	aotg_hcd_powergate_on(acthcd);
	aotg_hcd_wait_reset(acthcd);
	/* fpga : new DMA mode */
	...
	/*Writeb(acthcd->base + USBH_BKDOOR, (Readb(USBH_BKDOOR)| 0x04));*/
	/*set the tx_pre*/
...
static void aotg_hcd_init(struct usb_hcd *hcd, int id)
{
	struct aotg_hcd *acthcd = hcd_to_aotg(hcd);
	int i;
	...
	acthcd->tasklet_retry = 0;
	acthcd->port_specific = &aotg_data[id];
	acthcd->port = 0;
	acthcd->rhstate = AOTG_RH_POWEROFF;
	acthcd->inserted = 0;
...
static struct of_device_id aotg_hcd_of_match[] = {
...
static int aotg_hcd_get_dts(struct platform_device *pdev)
{
	...
	if (!of_find_property(of_node, "vbus_otg_en_gpio", NULL)) {
		...
		if (gpio_request(vbus_otg_en_gpio[pdev->id][0],
				aotg_hcd_of_match[pdev->id].compatible)) {
...
static int aotg_hub_probe(struct platform_device *pdev)
{
	...
	pdev->id = (res_mem->start == USBH_BASE0) ? 0 : 1;
...
#ifdef	CONFIG_PM
static int aotg_hcd_hub_suspend(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct aotg_hcd *acthcd = act_hcd_ptr[pdev->id];
	...
	dev_info(dev, " ==> %s\n", __func__);
	aotg_disable_irq(acthcd);
	aotg_hcd_powergate_off(acthcd);
	aotg_power_onoff(acthcd->id, 0);
	return 0;
}

static int aotg_hcd_hub_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct aotg_hcd *acthcd = act_hcd_ptr[pdev->id];
	struct usb_device *udev;
...
#ifdef CONFIG_PM
static const struct dev_pm_ops aotg_dev_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(aotg_hcd_hub_suspend, aotg_hcd_hub_resume)
};
#define DEV_PM_OPS	(&aotg_dev_pm_ops)
#else
...
{
	struct aotg_hcd *acthcd = act_hcd_ptr[dev_id];
	struct usb_hcd *hcd = aotg_to_hcd(acthcd);
	struct aotg_hcep *ep;
	int i;
	...
	if (!port_host_plug_detect[acthcd->id])
		aotg_power_onoff(acthcd->id, 0);
	acthcd->hcd_exiting = 1;
	pr_warn("usbh%d had been poweroff...\n", dev_id);
...
static int __init ahcd_init(void)
{
	...
	td_cache = kmem_cache_create(platform_drv_name, sizeof(struct aotg_td),
