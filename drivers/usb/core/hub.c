...
#ifdef CONFIG_OWL_USB3PHY
					/*When plug out, portstatus will set to inactive & connected,
					   this will lead to reset device.
					   If a udisk is connected, it will cause hangup for 30s
					   before scsi time out.
					   So we wait for 500ms to double check  portstatus to see
					   if it's really a plug out or not.
					*/
					if (usb3_need_double_check_inactive()) {
						u16 tmp_ps, tmp_pc, k;
						if ((portstatus & USB_PORT_STAT_LINK_STATE) == USB_SS_PORT_LS_SS_INACTIVE) {
							for (k = 0; k < 10; k++) {
								msleep(50);
								hub_port_status(hub, i , &tmp_ps, &tmp_pc);
								if ((tmp_ps & USB_PORT_STAT_LINK_STATE) != USB_SS_PORT_LS_SS_INACTIVE)
									break;
							}
							if (k < 10)
								continue;
