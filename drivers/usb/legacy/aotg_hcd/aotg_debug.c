...
#if 1	/* tmp test net disconnet bug. */
...
void aotg_hcd_show_single_ep_reg(struct aotg_hcd *acthcd, struct aotg_hcep *ep)
{
	...
	printk(" USBCLK(0x%x) : 0x%x\n",
			CMU_USBCLK, usb_readl((void *)CMU_USBCLK));
	printk("\n");
	...
	printk("HCIN07ERRIEN(0x%p) : 0x%X\n",
			acthcd->base + HCIN07ERRIEN, usb_readb(acthcd->base + HCIN07ERRIEN));
	printk("HCIN07ERRIRQ(0x%p) : 0x%X\n",
			acthcd->base + HCIN07ERRIRQ, usb_readb(acthcd->base + HCIN07ERRIRQ));
	printk("HCOUT07ERRIEN(0x%p) : 0x%X\n",
			acthcd->base + HCOUT07ERRIEN, usb_readb(acthcd->base + HCOUT07ERRIEN));
	printk("HCOUT07ERRIRQ(0x%p) : 0x%X\n",
			acthcd->base + HCOUT07ERRIRQ, usb_readb(acthcd->base + HCOUT07ERRIRQ));
	printk("\n");
	...
	if (ep->is_out) {
		if (1 == ep->index){
			...
			printk(" HCOUT1MAXPCKL(0x%p) : 0x%X\n",
					acthcd->base + HCOUT1MAXPCKL, usb_readw(acthcd->base + HCOUT1MAXPCKL));
			...
			printk(" HCOUT2MAXPCKL(0x%p) : 0x%X\n",
					acthcd->base + HCOUT2MAXPCKL, usb_readw(acthcd->base + HCOUT2MAXPCKL));

		} else if (3 == ep->index) {
			printk(" HCOUT3BC(0x%p) : 0x%X\n",
					acthcd->base + HCOUT3BCL, usb_readw(acthcd->base + HCOUT3BCL));
			printk(" HCOUT3CON(0x%p) : 0x%X\n",
					acthcd->base + HCOUT3CON, usb_readb(acthcd->base + HCOUT3CON));
			printk(" HCOUT3CS(0x%p) : 0x%X\n",
					acthcd->base + HCOUT3CS, usb_readb(acthcd->base + HCOUT3CS));
			printk(" HCOUT3CTRL(0x%p) : 0x%X\n",
					acthcd->base + HCOUT3CTRL, usb_readb(acthcd->base + HCOUT3CTRL));
			printk("HCOUT3ERR(0x%p) : 0x%X\n",
					acthcd->base + HCOUT3ERR, usb_readb(acthcd->base + HCOUT3ERR));
			printk(" HCOUT3STARTADDRESS(0x%p) : 0x%X\n",
					acthcd->base + IN3STARTADDRESS, usb_readl(acthcd->base + IN3STARTADDRESS));
			printk(" HCOUT3MAXPCKL(0x%p) : 0x%X\n",
					acthcd->base + HCOUT3MAXPCKL, usb_readw(acthcd->base + HCOUT3MAXPCKL));
		} else if (4 == ep->index) {
			printk(" HCOUT4BC(0x%p) : 0x%X\n",
					acthcd->base + HCOUT4BCL, usb_readw(acthcd->base + HCOUT4BCL));
			printk(" HCOUT4CON(0x%p) : 0x%X\n",
					acthcd->base + HCOUT4CON, usb_readb(acthcd->base + HCOUT4CON));
			printk(" HCOUT4CS(0x%p) : 0x%X\n",
					acthcd->base + HCOUT4CS, usb_readb(acthcd->base + HCOUT4CS));
			printk(" HCOUT4CTRL(0x%p) : 0x%X\n",
					acthcd->base + HCOUT4CTRL, usb_readb(acthcd->base + HCOUT4CTRL));
			printk("HCOUT4ERR(0x%p) : 0x%X\n",
					acthcd->base + HCOUT4ERR, usb_readb(acthcd->base + HCOUT4ERR));
			printk(" HCOUT4STARTADDRESS(0x%p) : 0x%X\n",
					acthcd->base + IN4STARTADDRESS, usb_readl(acthcd->base + IN4STARTADDRESS));
			printk(" HCOUT4MAXPCKL(0x%p) : 0x%X\n",
					acthcd->base + HCOUT4MAXPCKL, usb_readw(acthcd->base + HCOUT4MAXPCKL));
		} else if (5  ==  ep->index) {
			printk(" HCOUT5BC(0x%p) : 0x%X\n",
					acthcd->base + HCOUT5BCL, usb_readw(acthcd->base + HCOUT5BCL));
			printk(" HCOUT5CON(0x%p) : 0x%X\n",
					acthcd->base + HCOUT5CON, usb_readb(acthcd->base + HCOUT5CON));
			printk(" HCOUT5CS(0x%p) : 0x%X\n",
					acthcd->base + HCOUT5CS, usb_readb(acthcd->base + HCOUT5CS));
			printk(" HCOUT5CTRL(0x%p) : 0x%X\n",
					acthcd->base + HCOUT5CTRL, usb_readb(acthcd->base + HCOUT5CTRL));
			printk("HCOUT5ERR(0x%p) : 0x%X\n",
					acthcd->base + HCOUT5ERR, usb_readb(acthcd->base + HCOUT5ERR));
			printk(" HCOUT5STARTADDRESS(0x%p) : 0x%X\n",
					acthcd->base + IN5STARTADDRESS, usb_readl(acthcd->base + IN5STARTADDRESS));
			printk(" HCOUT5MAXPCKL(0x%p) : 0x%X\n",
					acthcd->base + HCOUT5MAXPCKL, usb_readw(acthcd->base + HCOUT5MAXPCKL));
		} else
			ACT_HCD_ERR
	} else if (!ep->is_out) {
		if (1 == ep->index) {
			...
			printk("HCIN1ERR(0x%p) : 0x%X\n",
					acthcd->base + HCIN1ERR, usb_readb(acthcd->base + HCIN1ERR));
			printk(" OUT1STARTADDRESS(0x%p) : 0x%X\n",
					acthcd->base + OUT1STARTADDRESS, usb_readl(acthcd->base + OUT1STARTADDRESS));
			printk(" HCIN1MAXPCKL(0x%p) : 0x%X\n",
		            acthcd->base + HCIN1MAXPCKL, usb_readw(acthcd->base + HCIN1MAXPCKL));
		} else if (2 == ep->index) {
			...
			printk("HCIN2ERR(0x%p) : 0x%X\n",
					acthcd->base + HCIN2ERR, usb_readb(acthcd->base + HCIN2ERR));
			printk(" OUT2STARTADDRESS(0x%p) : 0x%X\n",
			    acthcd->base + OUT2STARTADDRESS, usb_readl(acthcd->base + OUT2STARTADDRESS));
			printk(" HCIN2MAXPCKL(0x%p) : 0x%X\n",
		            acthcd->base + HCIN2MAXPCKL, usb_readw(acthcd->base + HCIN2MAXPCKL));
		} else if (3 == ep->index) {
			printk(" HCIN3BC(0x%p) : 0x%X\n",
		            acthcd->base + HCIN3BCL, usb_readw(acthcd->base + HCIN3BCL));
			printk(" HCIN3_COUNTL(0x%p) : 0x%X\n",
		            acthcd->base + HCIN3_COUNTL, usb_readw(acthcd->base + HCIN3_COUNTL));
			printk(" HCIN3CON(0x%p) : 0x%X\n",
		            acthcd->base + HCIN3CON, usb_readb(acthcd->base + HCIN3CON));
			printk(" HCIN3CS(0x%p) : 0x%X\n",
		            acthcd->base + HCIN3CS, usb_readb(acthcd->base + HCIN3CS));
			printk(" HCIN3CTRL(0x%p) : 0x%X\n",
		            acthcd->base + HCIN3CTRL, usb_readb(acthcd->base + HCIN3CTRL));
			printk("HCIN3ERR(0x%p) : 0x%X\n",
					acthcd->base + HCIN3ERR, usb_readb(acthcd->base + HCIN3ERR));
			printk(" OUT3STARTADDRESS(0x%p) : 0x%X\n",
			    acthcd->base + OUT3STARTADDRESS, usb_readl(acthcd->base + OUT3STARTADDRESS));
			printk(" HCIN3MAXPCKL(0x%p) : 0x%X\n",
		            acthcd->base + HCIN3MAXPCKL, usb_readw(acthcd->base + HCIN3MAXPCKL));
		} else if (4 == ep->index) {
			printk(" HCIN4BC(0x%p) : 0x%X\n",
		            acthcd->base + HCIN4BCL, usb_readw(acthcd->base + HCIN4BCL));
			printk(" HCIN4_COUNTL(0x%p) : 0x%X\n",
		            acthcd->base + HCIN4_COUNTL, usb_readw(acthcd->base + HCIN4_COUNTL));
			printk(" HCIN4CON(0x%p) : 0x%X\n",
		            acthcd->base + HCIN4CON, usb_readb(acthcd->base + HCIN4CON));
			printk(" HCIN4CS(0x%p) : 0x%X\n",
		            acthcd->base + HCIN4CS, usb_readb(acthcd->base + HCIN4CS));
			printk(" HCIN4CTRL(0x%p) : 0x%X\n",
		            acthcd->base + HCIN4CTRL, usb_readb(acthcd->base + HCIN4CTRL));
			printk("HCIN4ERR(0x%p) : 0x%X\n",
					acthcd->base + HCIN4ERR, usb_readb(acthcd->base + HCIN4ERR));
			printk(" OUT4STARTADDRESS(0x%p) : 0x%X\n",
			    acthcd->base + OUT4STARTADDRESS, usb_readl(acthcd->base + OUT4STARTADDRESS));
			printk(" HCIN4MAXPCKL(0x%p) : 0x%X\n",
		            acthcd->base + HCIN4MAXPCKL, usb_readw(acthcd->base + HCIN4MAXPCKL));
		} else if (5 == ep->index) {
			printk(" HCIN5BC(0x%p) : 0x%X\n",
		            acthcd->base + HCIN5BCL, usb_readw(acthcd->base + HCIN5BCL));
			printk(" HCIN5_COUNTL(0x%p) : 0x%X\n",
		            acthcd->base + HCIN5_COUNTL, usb_readw(acthcd->base + HCIN5_COUNTL));
			printk(" HCIN5CON(0x%p) : 0x%X\n",
		            acthcd->base + HCIN5CON, usb_readb(acthcd->base + HCIN5CON));
			printk(" HCIN5CS(0x%p) : 0x%X\n",
		            acthcd->base + HCIN5CS, usb_readb(acthcd->base + HCIN5CS));
			printk(" HCIN5CTRL(0x%p) : 0x%X\n",
		            acthcd->base + HCIN5CTRL, usb_readb(acthcd->base + HCIN5CTRL));
			printk("HCIN5ERR(0x%p) : 0x%X\n",
					acthcd->base + HCIN5ERR, usb_readb(acthcd->base + HCIN5ERR));
			printk(" OUT5STARTADDRESS(0x%p) : 0x%X\n",
			    acthcd->base + OUT5STARTADDRESS, usb_readl(acthcd->base + OUT5STARTADDRESS));
			printk(" HCIN5MAXPCKL(0x%p) : 0x%X\n",
		            acthcd->base + HCIN5MAXPCKL, usb_readw(acthcd->base + HCIN5MAXPCKL));
		} else
			ACT_HCD_ERR
	}
...
static int aotg_hcd_show_enque_info(struct seq_file *s, struct aotg_hcd	*acthcd)
{
	...
	for (i = 1; i < MAX_EP_NUM; i++) {
		...
		if (ep) {
			...
			seq_printf(s,"ep_timeout_cnt:%d\n", ep->ep_timeout_cnt);
			seq_printf(s,"ep_reset_cnt:%d\n", ep->ep_reset_cnt);

#endif
			if(PIPE_INTERRUPT == ep->type){
				seq_printf(s,"intr_recall_cnt:%d\n", ep->intr_recall_cnt);
			}
			seq_printf(s, "ep->epnum:%d\n", ep->epnum);

			list_for_each_entry_safe(q, next, &ep->q_list, ep_q_list) {
				seq_printf(s, "ep->index: %d, type: %d, dir : %s,transfer_type 	: %s, transfer_buffer_length: %d, actual_length:%d\n",
					q->ep->index,	usb_pipetype(q->urb->pipe), usb_pipeout(q->urb->pipe)?"out":"in",ep->is_use_pio?"PIO":"DMA",
					q->urb->transfer_buffer_length, q->urb->actual_length);
			}
		}
	}
...
#endif
			if(ep->is_use_pio == 0){
				seq_printf(s, "dma_handler_cnt:%u\n", ep->dma_handler_cnt);
				seq_printf(s, "dma_outirq_cnt:%u\n", ep->dma_outirq_cnt);
				seq_printf(s, "dma_timeout_cnt:%u\n", ep->dma_timeout_cnt);
			}
			list_for_each_entry_safe(q, next, &ep->q_list, ep_q_list) {
				seq_printf(s, "ep->index: %d, type: %d, dir : %s, transfer_type : %s, transfer_buffer_length: %d, actual_length:%d\n",
					q->ep->index,	usb_pipetype(q->urb->pipe), usb_pipeout(q->urb->pipe)?"out":"in",ep->is_use_pio?"PIO":"DMA",
...
void aotg_hcd_show_single_queue_info(struct aotg_hcd *acthcd, struct aotg_hcep *ep)
{
	struct aotg_queue *q, *next;
	...
	if(ep->is_out){
		...
		printk("ep->epnum:%d\n", ep->epnum);
#ifdef EP_TIMEOUT_DISCONNECT
		printk("ep_timeout_cnt:%d\n", ep->ep_timeout_cnt);
		printk("ep_reset_cnt:%d\n", ep->ep_reset_cnt);

#endif
		if(ep->is_use_pio == 0){
			printk("dma_handler_cnt:%u\n", ep->dma_handler_cnt);
			printk("dma_outirq_cnt:%u\n", ep->dma_outirq_cnt);
			printk("dma_timeout_cnt:%u\n", ep->dma_timeout_cnt);
		}
		list_for_each_entry_safe(q, next, &ep->q_list, ep_q_list) {
			printk("ep->index: %d, type: %d, dir : %s, transfer_type : %s, transfer_buffer_length: %d, actual_length:%d\n",
				q->ep->index,	usb_pipetype(q->urb->pipe), usb_pipeout(q->urb->pipe)?"out":"in",ep->is_use_pio?"PIO":"DMA",
				q->urb->transfer_buffer_length, q->urb->actual_length);
		}
	}
...
int aotg_hcd_proc_show(struct seq_file *s, void *unused)
{
	...
	if (aotg_hcd_proc_sign == 'd') {
		i = aotg_hcd_dma_get_cmd(acthcd->dma_nr0);
		if (((i & 0x1) == 0) && (acthcd->dma_working[0] != 0)) {
			printk("i:%d\n", i);
		}
		i = aotg_hcd_dma_get_cmd(acthcd->dma_nr1);
		...
		if (acthcd->dma_nr0 >= 0) {
			...
			seq_printf(s, "aotg_hcd_dma_get_cmd : %d\n", aotg_hcd_dma_get_cmd(acthcd->dma_nr0));
			seq_printf(s, "dma1 remain : %d\n", aotg_hcd_dma_get_remain(acthcd->dma_nr1));
...
static struct proc_dir_entry *acts_hcd_pde;
...
static int wifi_open_cnt = 0;
static int bt_open_cnt = 0;

ssize_t acts_hcd_proc_write(struct file *file, const char __user *buf,
					size_t count, loff_t *ppos)
{
	...
	if (c == 'a') {
		aotg_force_init_hub(0);
		printk("hcd aotg0 add\n");
		aotg0_device_init(0);
	}
	if (c == 'b') {
		aotg_force_exit_hub(0);
	...
	if (c == 'c') {
	    if (bt_open_cnt == 0) {
			bt_open_cnt = 1;
		    printk("hcd aotg1 bt add\n");
		    aotg1_device_init(0);
		} else {
		    printk("hcd aotg1 bt already add!\n");
		}
	}
	if (c == 'd') {
	    if (bt_open_cnt == 1) {
			bt_open_cnt = 0;
		    printk("hcd aotg1 bt remove\n");
		    aotg1_device_exit(0);
		} else {
		    printk("hcd aotg1 bt already remove!\n");
		}
	}
	...
	if (c == 'j') {
	    ...
	    if (wifi_open_cnt == 0) {
			wifi_open_cnt = 1;
		    printk("hcd aotg1 wifi add\n");
		    aotg1_device_init(0);
		} else {
		    printk("hcd aotg1 wifi already add!\n");
		}
	}
	if (c == 'k') {
	    ...
	    if (wifi_open_cnt == 1) {
			wifi_open_cnt = 0;
		    printk("hcd aotg1 wifi remove\n");
		    aotg1_device_exit(0);
		} else {
		    printk("hcd aotg1 wifi already remove!\n");
		}
	}
...
{
	acts_hcd_pde = proc_create_data("acts_hcd", 0, NULL,
					&acts_proc_ops, acts_hcd_pde);
	return;
}
