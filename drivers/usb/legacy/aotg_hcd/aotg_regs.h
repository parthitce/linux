...
#define AOTG0_BASE              0xB0240000
#define AOTG1_BASE              0xB0250000
...
#if 0
/* ioremap */
static void inline usb_writeb(u8 val, void __iomem *reg)
{
	writeb(val, reg);
}

static void inline usb_writew(u16 val, void __iomem *reg)
{
	writew(val, reg);
}

static void inline usb_writel(u32 val, void __iomem *reg)
{
	writel(val, reg);
}
...
static void inline usb_setb(u8 val, void __iomem *reg)
{
	//act_setb(val, reg);
	u8 temp = *(volatile u8 *)reg;
	*(volatile u8 *)reg = temp | val;
}

static void inline usb_setw(u16 val, void __iomem *reg)
{
	//act_setw(val, reg);
	u16 temp = *(volatile u16 *)reg;
	*(volatile u16 *)reg = temp | val;
}

static void inline usb_setl(u32 val, void __iomem *reg)
{
	//act_setl(val, reg);
	u32 temp = *(volatile u32 *)reg;
	*(volatile u32 *)reg = temp | val;
}

static void inline usb_clearb(u8 val, void __iomem *reg)
{
	//act_clearb(val, reg);
	u8 temp = *(volatile u8 *)reg;
	*(volatile u8 *)reg = temp | ~val;
...
static void inline usb_clearw(u16 val, void __iomem *reg)
{
	//act_clearw(val, reg);
	u16 temp = *(volatile u16 *)reg;
	*(volatile u16 *)reg = temp | ~val;
...
static void inline usb_clearl(u32 val, void __iomem *reg)
{
	//act_clearl(val, reg);
	u32 temp = *(volatile u32 *)reg;
	*(volatile u32 *)reg = temp | ~val;
...
static inline void usb_setbitsl(u32 mask, void __iomem *mem)
{
...
static inline void usb_clearbitsl(u32 mask, void __iomem *mem)
{
	usb_writel(usb_readl(mem) & ~mask, mem);
}
