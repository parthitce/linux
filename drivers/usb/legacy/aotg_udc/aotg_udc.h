...
#define EP_ISO_IN_STARTADD		0x00000080
#define EP_INT_IN_STARTADD		0x00000C80
...
struct aotg_ep {
	...
	u8						mask;
	u8						buftype;
	u8						dma_channel_number; /*by yl, show which dma channel is used by the ep*/
	u32						dma_bytes;
...
struct aotg_udc {
	...
	int		dma_nr0;	// for ep-in: UDMA1
	int		dma_nr1;	// for ep-out: UDMA2
	...
	struct usb_phy	*transceiver;	// phy-related
...
#define extern_irq_disable(udc)	udc_clearb_mask(udc, USBEIEN, USB_EXTENAL_IRQ_ENABLE)
...
#ifdef DEBUG_UDC
#define UDC_DEBUG(fmt, args...)	pr_debug("<UDC %s:%d>"fmt, __func__, __LINE__, ## args)
#else
#define UDC_DEBUG(fmt, args...)	/* not printk */
#endif

#define UDC_NOTICE(fmt, args...)	pr_notice("<UDC %s:%d>"fmt, __func__, __LINE__, ## args)

#define UDC_PRINTK(fmt, args...)


#ifdef ERR_UDC
#define UDC_ERR(fmt, args...)	pr_err("<UDC %s:%d>"fmt, __func__, __LINE__, ## args)
#else
#define UDC_ERR(fmt, args...)	/* not printk */
#endif

#ifdef ERR_UDC_DMA
#define UDC_DMA_DEBUG(fmt, args...)	pr_err(fmt, ## args)
#else
#define UDC_DMA_DEBUG(fmt, args...)	/*not printk*/
#endif

#define UDC_WARNING(fmt, args...)	pr_warn(fmt, ## args)
