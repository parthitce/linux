...
static unsigned int plug_mask;
static unsigned int plug_test;
#if INPUT_MODE_DETECT
static unsigned long input_interval;
#endif

static int work_sequence_on;
...
static __cpuinit void cpu_hot_plug_test_for_stability(void)
{
	...
	if (cpu_possible(3) && test_cpu_not_mask(3)) {
		if (cpu_online(3) == CPU_OFF) {
			pr_debug("cpu3 turning on start...");
			cpu_up(3);
			hotpluging_rate = cpu_plug_rate_def[nr_online_cpu].up_rate;
			pr_debug("end\n");
		} else {
			pr_debug("cpu3 turning off start...");
			cpu_down(3);
	...
	if (cpu_possible(2) && test_cpu_not_mask(2)) {
		if (cpu_online(2) == CPU_OFF) {
			pr_debug("cpu2 turning on start...");
			cpu_up(2);
			hotpluging_rate = cpu_plug_rate_def[nr_online_cpu].up_rate;
			pr_debug("end\n");
		} else {
			pr_debug("cpu2 turning off start...");
			cpu_down(2);
			hotpluging_rate = cpu_plug_rate_def[nr_online_cpu-1].down_rate;
			pr_debug("end\n");
		}
	...
	if (test_cpu_not_mask(1)) {
		if (cpu_online(1) == CPU_OFF) {
			pr_debug("cpu1 turning on start...");
			cpu_up(1);
			pr_debug("end\n");
		} else {
			pr_debug("cpu1 turning off start...");
			cpu_down(1);
			pr_debug("end\n");
		}
	}
...
int force_plugout;
int owl_hotplug_force_plugout(int enable)
{
	force_plugout = enable;
	return 0;
}
...
static __cpuinit void hotplug_timer(struct work_struct *work)
{
	...
	if (!force_plugout)
		...
		pr_debug("%s, force_plugout\n", __func__);
		if (cpu_online(3) == CPU_ON)
			cpu_down(3);
		if (cpu_online(2) == CPU_ON)
			cpu_down(2);
		flag_hotplug = HOTPLUG_NOP;
	}
	/*cpu hotplug*/
...
/*	struct freq_tran_work *freq_tran_workp = (struct freq_tran_work*)work;
		...
		&& (freq_tran_workp->new > freq_threshold_for_cpuplug))
	{
		...
		work_sequence_on = 1;
		mutex_unlock(&hotplug_lock);
		pr_debug("enable cpu_hot_plug, freq: %d, thresh: %d\n", freq_tran_workp->new, freq_max/2);
	}
	else
	{
		mutex_lock(&hotplug_lock);
		freq_lock = 1;
		pr_debug("freq_lock: %d, freq: %d, thresh: %d\n", freq_lock, freq_tran_workp->new, freq_max/2);
		mutex_unlock(&hotplug_lock);
	}
 */
}
...
static __cpuinit void early_suspend_process(struct work_struct *work)
{
	...
	switch (tmp->flag) {
	case UP_CPU1:
		pr_debug("early_suspend_process UP_CPU1 start ...");
/* 		if (cpu_online(1) == CPU_OFF)
			cpu_up(1); */
		mutex_lock(&hotplug_lock);
		fb_lock = 1;
	...
	break;
	case DOWN_CPU1:
		pr_debug("early_suspend_process DOWN start ...");
