...
#include "owl_drm_lcd.h"
#include "owl_drm_iommu.h"

static struct device *g_dev = NULL;
static bool lcd_display_is_connected(struct device *dev)
{
	DRM_DEBUG_KMS("\n");
...
static void *lcd_get_panel(struct device *dev)
{
	struct lcd_context *ctx = get_lcd_context(dev);
...
static int lcd_check_timing(struct device *dev, void *timing)
{
	DRM_DEBUG_KMS("\n");
...
static int lcd_display_power_on(struct device *dev, int mode)
{
	struct lcd_context *ctx = get_lcd_context(dev);
...
static struct owl_drm_display_ops lcd_display_ops = {
	.type 		= OWL_DRM_DISPLAY_TYPE_LCD,
	.is_connected 	= lcd_display_is_connected,
	.get_panel 	= lcd_get_panel,
	.check_timing 	= lcd_check_timing,
	.power_on 	= lcd_display_power_on,
};

static void lcd_path_dpms(struct device *subdrv_dev, int mode)
{
	struct lcd_context *ctx = get_lcd_context(subdrv_dev);
	...
	switch (mode) {
		case DRM_MODE_DPMS_ON:
			 ...
			 *
			 * P.S. lcd_dpms function would be called at booting time so
...
static void lcd_path_mode_set(struct device *subdrv_dev)
{
	struct lcd_context *ctx  = get_lcd_context(subdrv_dev);
	struct owl_de_path *path = ctx->path;
	struct owl_panel *panel  = ctx->panel;
...
static void lcd_path_apply(struct device *subdrv_dev)
{
	struct lcd_context *ctx = get_lcd_context(subdrv_dev);
	struct owl_de_path *path = ctx->path;
	int i;
...
static void lcd_path_prepare(struct device *dev)
{
	struct lcd_context *ctx = get_lcd_context(dev);
...
static void lcd_path_commit(struct device *dev)
{
	struct lcd_context *ctx = get_lcd_context(dev);
...
static int lcd_path_enable_vblank(struct device *dev)
{
	struct lcd_context *ctx = get_lcd_context(dev);
	struct owl_de_path *path = ctx->path;
...
static void lcd_path_disable_vblank(struct device *dev)
{
	struct lcd_context *ctx = get_lcd_context(dev);
	struct owl_de_path *path = ctx->path;
...
static void lcd_path_wait_for_vblank(struct device *dev)
{
	struct lcd_context *ctx = get_lcd_context(dev);
...
static void lcd_vsync_event_cb(struct owl_panel *panel, void *data,
		u32 value)
{
	struct lcd_context *ctx 	= data;
...
static struct owl_drm_manager_ops lcd_manager_ops = {
	.dpms 			= lcd_path_dpms,
	.prepare 		= lcd_path_prepare,

	.mode_set 		= lcd_path_mode_set,
	.commit 		= lcd_path_commit,

	.enable_vblank 		= lcd_path_enable_vblank,
	.disable_vblank 	= lcd_path_disable_vblank,
	.wait_for_vblank 	= lcd_path_wait_for_vblank,
};

enum owl_color_mode lcd_drm_format_to_owl_format(u32 color)
{
...
#undef DIV_ROUND
	...
	info->color_mode = lcd_drm_format_to_owl_format(overlay->pixel_format);
	...
	for (plane = 0; plane < info->n_planes; plane++) {
		...
		info->addr[plane] = overlay->dma_addr[plane] + info->offset[0];

		DRM_DEBUG_KMS("%s: plane %d, Pitch %d, Offset %d, Addr %ld\n", __func__,
...
static void lcd_video_mode_set(struct device *dev,
		struct owl_drm_overlay *overlay)
{
	struct lcd_context *ctx = get_lcd_context(dev);
	struct owl_de_path *path = ctx->path;
...
static void lcd_win_set_pixfmt(struct device *dev, unsigned int win)
{
	struct lcd_context *ctx = get_lcd_context(dev);
}

static void lcd_win_set_colkey(struct device *dev, unsigned int win)
{
	DRM_DEBUG_KMS("\n");
}

static void lcd_video_apply(struct device *dev, int zpos)
{
	struct lcd_context *ctx = get_lcd_context(dev);
...
static void lcd_video_commit(struct device *dev, int zpos)
{
	struct lcd_context *ctx = get_lcd_context(dev);
	struct owl_de_path *path = ctx->path;
	struct owl_de_video *video;
...
static void lcd_video_enable(struct device *dev, int zpos)
{
	struct lcd_context *ctx = get_lcd_context(dev);
	int win = zpos;
...
static void lcd_video_disable(struct device *dev, int zpos)
{
	struct lcd_context *ctx = get_lcd_context(dev);
	int win = zpos;
...
static struct owl_drm_overlay_ops lcd_overlay_ops = {
	.mode_set 	= lcd_video_mode_set,
	.commit 	= lcd_video_commit,

	/*
	 * this callback not used, FIXME
	 * */
	.apply		= lcd_video_apply,
	.enable		= lcd_video_enable,
	.disable	= lcd_video_disable,
};

static struct owl_drm_manager lcd_manager = {
	.pipe		= -1,
	.ops		= &lcd_manager_ops,
	.overlay_ops	= &lcd_overlay_ops,
	.display_ops	= &lcd_display_ops,
};

static int lcd_subdrv_probe(struct drm_device *drm_dev, struct device *dev)
{
	DRM_DEBUG_KMS("\n");
...
static void lcd_subdrv_remove(struct drm_device *drm_dev, struct device *dev)
{
	DRM_DEBUG_KMS("\n");
...
static void lcd_clear_win(struct lcd_context *ctx, int win)
{
	DRM_DEBUG_KMS("\n");
}

static void lcd_window_suspend(struct device *dev)
{
	struct lcd_context *ctx = get_lcd_context(dev);
	int i;

	for (i = 0; i < WINDOWS_NR; i++)
		lcd_video_disable(dev, i);

	lcd_path_wait_for_vblank(dev);
}

static void lcd_window_resume(struct device *dev)
{
	struct lcd_context *ctx = get_lcd_context(dev);
	int i;
...
static int lcd_activate(struct lcd_context *ctx, bool enable)
{
...
static int lcd_probe(struct platform_device *pdev)
{
	...
	/* get path */
	path = owl_de_path_get_by_index(1);
	...
	owl_panel_vsync_cb_set(ctx->panel, lcd_vsync_event_cb, ctx);
	...
	subdrv->dev = dev;
	subdrv->manager = &lcd_manager;
	subdrv->probe = lcd_subdrv_probe;
	subdrv->remove = lcd_subdrv_remove;
...
static int lcd_resume(struct device *dev)
{
	...
	if (!pm_runtime_suspended(dev)) {
		...
		ret = lcd_activate(ctx, true);
		if (ret < 0)
			return ret;

		/*
		 * in case of dpms on(standby), lcd_path_apply function will
		 * be called by encoder's dpms callback to update fimd's
		 * registers but in case of sleep wakeup, it's not.
		 * so lcd_path_apply function should be called at here.
		 */
	}
...
static int lcd_runtime_suspend(struct device *dev)
{
	struct lcd_context *ctx = get_lcd_context(dev);
...
static int lcd_runtime_resume(struct device *dev)
{
	struct lcd_context *ctx = get_lcd_context(dev);

	DRM_DEBUG_KMS("\n");

	return lcd_activate(ctx, true);
}
#endif

static const struct dev_pm_ops lcd_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(lcd_suspend, lcd_resume)
		SET_RUNTIME_PM_OPS(lcd_runtime_suspend, lcd_runtime_resume, NULL)
};
