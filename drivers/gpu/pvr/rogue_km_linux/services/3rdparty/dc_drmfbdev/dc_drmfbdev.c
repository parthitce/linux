...
static bool FFBFlipPossible(struct fb_info *ffbInfo)
{
    ...
    pr_err(DRVNAME "line=%d, func=%s, yres=%d, vyres=%d, num=%d\n",
        __LINE__, __FUNCTION__,
        sVar.yres, sVar.yres_virtual, NUM_PREFERRED_BUFFERS);
...
int dc_drmfbdev_buffer_alloc(void *display_priv, size_t size, struct pvr_drm_display_buffer **buffer_out)
{
	...
	if ((devInfo->ffbInfo->fix.line_length * sVar.yres) != size)
	{
        ...
        pr_err(DRVNAME "line_length: %llu, yres: %llu",
            (unsigned long long)(devInfo->ffbInfo->fix.line_length),
            (unsigned long long)(sVar.yres));
		return -EINVAL;
	}
