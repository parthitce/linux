...
int mali_mem_os_alloc_pages(mali_mem_os_mem *os_mem, u32 size)
{
	...
	for (i = 0; i < remaining; i++) {
		...
		MALI_DEBUG_PRINT(5, ("allocing page %d: allocing,remaining %d pages\n", i, remaining - i));
		if (unlikely(NULL == new_page)) {
			MALI_DEBUG_PRINT(1, ("alloc_page: failed\n"));
		...
		MALI_DEBUG_PRINT(5, ("alloc_page %d: success,remaining %d pages\n", i, remaining - i));
