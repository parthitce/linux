...
#define MALI_UTILIZATION_TO_PERCENT(utilization) ((int) ((utilization)*(100)/MAX_PERFORMANCE_VALUE))
...
/*Function prototype */
static mali_bool (*mali_gpu_set_freq)(unsigned int) = NULL;
static int (*mali_gpu_get_freq)(void) = NULL;

static mali_bool mali_dvfs_enabled = MALI_FALSE;
static u32 current_gpu_util = 0;
static u32 target_gpu_freq = 0;
...
static bool mali_pickup_closest_avail_clock(int target_clock_mhz, mali_bool pick_clock_up)
{
	...
	if (cur_clk_step != clock_step) {
		clock_changed = true;
		if (abs(clock_step - cur_clk_step) > 2) {
		    clock_step = clock_step > cur_clk_step ? (cur_clk_step + 1):(cur_clk_step - 1);
		    MALI_DEBUG_PRINT(3, ("pickup step upper or lower 2 than current step , recalc step = %d\n", clock_step));
		}
	}
...
void mali_dvfs_policy_realize(struct mali_gpu_utilization_data *data, u64 time_period)
{
	...
	MALI_DEBUG_PRINT(5, ("DVFS : cur_clk_step %d, current_fps %d, current_gpu_util %d\n", cur_clk_step, current_fps, current_gpu_util));
	/* Consider offscreen */
	if (0 == current_fps) {
		...
		if (current_gpu_util > over_perform_boundary_value) {
			if (cur_clk_step != gpu_clk->num_of_steps - 1) {
				clock_changed = true;
				if ((gpu_clk->num_of_steps - 1 - cur_clk_step) > 2) {
	...
	if (-1 != cur_clk_step) {
		...
		target_gpu_freq = gpu_clk->item[cur_clk_step].clock;
...
real_setting:
	if (clock_changed) {
		...
		if ((mali_clk_lock_to_step < gpu_clk->num_of_steps) && (mali_clk_lock_to_step >= 0))
		{
			clock_step = mali_clk_lock_to_step;
		}

		MALI_DEBUG_PRINT(3, ("DVFS : change clock to step %d\n", clock_step));
...
#if 0
	if (cur_clk_step != (gpu_clk->num_of_steps - 1)) {
		set_step = gpu_clk->num_of_steps - 1;
	}
#else
	/** just keep **/
	set_step = cur_clk_step;
#endif

	mali_gpu_set_freq(set_step);
	_mali_osk_profiling_add_event(MALI_PROFILING_EVENT_TYPE_SINGLE |
                      MALI_PROFILING_EVENT_CHANNEL_GPU |
                      MALI_PROFILING_EVENT_REASON_SINGLE_GPU_FREQ_VOLT_CHANGE, gpu_clk->item[set_step].clock,
                      gpu_clk->item[set_step].vol / 1000, 0, 0, 0);
}
...
u32 _mali_ukk_utilization_gpu(void)
{
    return MALI_UTILIZATION_TO_PERCENT(current_gpu_util);
}

u32 _mali_ukk_dvfs_target_freq(void)
{
    return target_gpu_freq;
}

u32 _mali_ukk_dvfs_cur_fps(void)
{
    return current_fps;
}
