...
static int _atc260x_pm_get_wakeup_flag(void)
{
	...
	/* read boot from s2 flag  that set by bootloader */
	ret = atc260x_pstore_get(atc260x_pm->atc260x,
				 ATC260X_PSTORE_TAG_FW_BOOT_FROM_S2,
				 &boot_from_s2);
	if (ret) {
		dev_err(atc260x_pm->dev, "%s() get_boot_from_s2 flag failed\n",
			__func__);
	}

	/* return empty wakeup flag if not boot from s2 */
	if (!boot_from_s2)
		return 0;
