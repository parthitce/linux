...
static const struct atc260x_cmu_reg_cfg *const
    sc_atc260x_cmu_reg_cfg_tbl[ATC260X_ICTYPE_CNT] = {
...
int atc260x_cmu_clk_ctrl(struct atc260x_dev *atc260x, uint cmu_module,
			 uint clk_en)
{
	struct atc260x_cmu_reg_cfg const *regcfg;
	...
	ret =
	    atc260x_reg_setbits(atc260x, reg_addr, (1U << clk_bit),
				(clk_en << clk_bit));
	if (ret < 0) {
		dev_err(atc260x->dev,
			"%s() faile to read/write cmu clock reg 0x%x\n",
			__func__, reg_addr);
		return -EIO;
	}
	return 0;
