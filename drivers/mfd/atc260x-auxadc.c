...
#define ATC260x_AUXADC_MAX_CHNL 21

struct atc260x_auxadc_chn_hwinfo;
typedef int (*atc260x_auxadc_getraw_func_t) (uint channel,
					     const struct
					     atc260x_auxadc_chn_hwinfo *
					     ch_info,
					     struct atc260x_dev * atc260x);
typedef int (*atc260x_auxadc_translate_func_t) (uint raw_val,
						const struct
						atc260x_auxadc_chn_hwinfo *
						ch_info,
						struct atc260x_dev * atc260x);
...
struct atc260x_auxadc_hwinfo {
	...
	const struct atc260x_auxadc_chn_hwinfo
	*chn_hwinfos[ATC260x_AUXADC_MAX_CHNL];
};
...
static const struct atc260x_auxadc_hwinfo sc_atc2603a_auxadc_hwinfo = {
	.channels = 16,
	.chn_names = {
		      "BATV", "BATI", "VBUSV", "VBUSI", "SYSPWRV", "WALLV",
		      "WALLI", "CHGI",
		      "IREF", "REMCON", "ICTEMP", "BAKBATV", "AUX0", "AUX1",
		      "AUX2", "AUX3"},
	.chn_unit_names = {
			   "mV", "mA", "mV", "mA", "mV", "mV", "mA", "mA",
			   "mV", "/1024", "mCel", "mV", "/1024", "/1024",
			   "/1024", "/1024"},
	.chn_hwinfos = {
...
static int _atc2603c_auxadc_getraw_icm_chn(uint channel, const struct
					   atc260x_auxadc_chn_hwinfo *ch_info,
					   struct atc260x_dev *atc260x)
{
	uint reg_val;
	int ret;
...
static void _atc2603c_auxadc_usertran_prepare_dbg_refs(struct atc260x_dev
						       *atc260x)
{
...
static int _atc2603c_auxadc_usertran_dbg_i_mode(uint raw_val, const struct
						atc260x_auxadc_chn_hwinfo
						*ch_info,
						struct atc260x_dev *atc260x)
{
...
static int _atc2603c_auxadc_usertran_dbg_rem_mode(uint raw_val, const struct
						  atc260x_auxadc_chn_hwinfo
						  *ch_info,
						  struct atc260x_dev *atc260x)
{
...
static int _atc2603c_auxadc_usertran_icm(uint raw_val, const struct atc260x_auxadc_chn_hwinfo
					 *ch_info, struct atc260x_dev *atc260x)
{
	struct atc260x_auxadc_udata_3c *pud;
	int tmp;
	pud = (struct atc260x_auxadc_udata_3c *)(atc260x->auxadc_udata);
...
static const struct atc260x_auxadc_chn_hwinfo
...
static int _atc2609a_auxadc_usertran_icm(uint raw_val, const struct atc260x_auxadc_chn_hwinfo
					 *ch_info, struct atc260x_dev *atc260x)
{
	...
	if (raw_val & 0x2000U)
		tmp = ((raw_val ^ 0x3fff) & 0x3fff) + 1;
	else
		tmp = raw_val & 0x1fffU;
	if (pud->icm_res_10ohm == 0) {
		tmp = (tmp * 9375U) >> 14;	/*tmp = tmp * 9375U / 16384U; */
	} else {
		tmp = (tmp * 9375U) >> 13;	/*tmp = tmp * 9375U / 8192U; */
	}
	if (raw_val & 0x2000U) {
		tmp = -tmp;
	}
	pr_debug("%s,icm raw:%d, trans:%d\n",__func__,raw_val,tmp);
	return tmp;
}

static int _atc2609a_auxadc_usertran_qmax(uint raw_val, const struct atc260x_auxadc_chn_hwinfo
					 *ch_info, struct atc260x_dev *atc260x)
{
	struct atc260x_auxadc_udata_9a *pud;
	int tmp;
	...
	pr_debug("%s,q raw:%d, trans:%d\n",__func__,raw_val,tmp);
	return tmp;
}

static int _atc2609a_auxadc_usertran_ictemp_adj(uint raw_val, const struct
						atc260x_auxadc_chn_hwinfo
						*ch_info,
						struct atc260x_dev *atc260x)
{
...
static const struct atc260x_auxadc_chn_hwinfo sc_atc2609a_chn_hwinfos_batv = {
	.dat_reg_addr = ATC2609A_PMU_ADC12B_V,
	.dat_bit_offs = 0,
	.dat_bit_cnt = 12,
	.tr_multiplier = 375,	/* raw * 0.732421875mv * 2 */
	.tr_r_shift = 8,
...
static const struct atc260x_auxadc_chn_hwinfo sc_atc2609a_chn_hwinfos_icm = {
	.dat_reg_addr = ATC2609A_PMU_ADC12B_I,
	.dat_bit_offs = 0,
	.dat_bit_cnt = 14,
	.tran_func = _atc2609a_auxadc_usertran_icm,
};

static const struct atc260x_auxadc_chn_hwinfo sc_atc2609a_chn_hwinfos_qmax = {
	.dat_reg_addr = ATC2609A_CLMT_DATA0,
	.dat_bit_offs = 0,
	.dat_bit_cnt = 16,
	.tran_func = _atc2609a_auxadc_usertran_qmax,
};

static const struct atc260x_auxadc_chn_hwinfo sc_atc2609a_chn_hwinfos_qrem = {
	.dat_reg_addr = ATC2609A_CLMT_DATA2,
	.dat_bit_offs = 0,
	.dat_bit_cnt = 16,
	.tran_func = _atc2609a_auxadc_usertran_qmax,
};

static const struct atc260x_auxadc_chn_hwinfo sc_atc2609a_chn_hwinfos_qavi = {
	.dat_reg_addr = ATC2609A_CLMT_DATA3,
	.dat_bit_offs = 0,
	.dat_bit_cnt = 16,
	.tran_func = _atc2609a_auxadc_usertran_qmax,
};
...
static const struct atc260x_auxadc_hwinfo sc_atc2609a_auxadc_hwinfo = {
	.channels = 21,
	.chn_names = {
		      "IREF", "CHGI", "VBUSI", "WALLI", "BATI", "REMCON",
		      "ICTEMP", "SVCC",
		      "BAKBATV", "SYSPWRV", "WALLV", "VBUSV", "AUX3", "AUX2",
		      "AUX1", "AUX0",
		      "BATV", "ICM", "QMAX", "QREM", "QAVI"},
	.chn_unit_names = {
			   "mV", "mA", "mA", "mA", "mA", "/4096", "mCel", "mV",
			   "mV", "mV", "mV", "mv", "/4096", "/4096", "/4096",
			   "/4096",
			   "mV", "mA", "mAH", "mAH", "mAH"},
	.chn_hwinfos = {
			...
			&sc_atc2609a_chn_hwinfos_icm,
			&sc_atc2609a_chn_hwinfos_qmax,
			&sc_atc2609a_chn_hwinfos_qrem,
			&sc_atc2609a_chn_hwinfos_qavi,
			},
};

int atc260x_auxadc_find_chan(struct atc260x_dev *atc260x,
			     const char *channel_name)
{
	...
	for (i = 0; i < atc260x->auxadc_hwinfo->channels; i++) {
		if (0 ==
		    strcmp(channel_name,
			   atc260x->auxadc_hwinfo->chn_names[i])) {
			return i;
		}
	}
	return -ENXIO;
...
const char *atc260x_auxadc_channel_name(struct atc260x_dev *atc260x,
					uint channel)
{
	ATC260X_ASSERT_VALID_DEV(atc260x);
...
const char *atc260x_auxadc_channel_unit_name(struct atc260x_dev *atc260x,
					     uint channel)
{
	ATC260X_ASSERT_VALID_DEV(atc260x);
	if (channel >= atc260x->auxadc_hwinfo->channels)
...
static int _atc260x_auxadc_get_raw_inner(struct atc260x_dev *atc260x,
					 uint channel,
					 const struct atc260x_auxadc_chn_hwinfo
					 *ch_info)
{
	...
	if (unlikely(ch_info->getraw_func)) {
		...
		raw_value =
		    ((uint) ret >> ch_info->dat_bit_offs) &
		    ((1U << ch_info->dat_bit_cnt) - 1U);
	}
	return raw_value;
}
...
int atc260x_auxadc_get_translated(struct atc260x_dev *atc260x, uint channel,
				  s32 * p_tr_value)
{
	const struct atc260x_auxadc_chn_hwinfo *ch_info;
	int ret, tr_value;
	...
	if (ch_info->tran_func) {
		...
		dev_dbg(atc260x->dev,
			"%s() channel #%u call 0x%lx for translate, result=%d\n",
			__func__, channel, (ulong) (ch_info->tran_func),
			tr_value);
	} else {
		tr_value =
		    (s32) (((s64) ret *
			    ch_info->tr_multiplier) >> ch_info->tr_r_shift) +
		    ch_info->tr_offset;
		dev_dbg(atc260x->dev,
			"%s() channel #%u linear translate, "
			"multiplier=%d r_shift=%u offset=%d result=%d\n",
			__func__, channel, ch_info->tr_multiplier,
			ch_info->tr_r_shift, ch_info->tr_offset, tr_value);
	}
...
static int _atc2603c_auxadc_dev_init(struct atc260x_dev *atc260x)
{
	...
	if (ic_ver == 0) {
		if (dat & (1U << 3)) {
			...
			atc260x_reg_write(atc260x, ATC2603C_PMU_AUXADC_CTL1,
					  dat & ~(1 << 3));
		}
	}
	/* open all channels (+ ~1uA) */
...
static int _atc2609a_auxadc_dev_init(struct atc260x_dev *atc260x)
{
	...
	/*setup value between CMN and CMP according to the resistor on board. */
	ret = atc260x_reg_setbits(atc260x, ATC2609A_PMU_SWCHG_CTL0,
				  (1U << 12), (((icm_ohm == 10) ? 1 : 0) << 12));
	if (ret) {
		goto label_io_err;
	}	
