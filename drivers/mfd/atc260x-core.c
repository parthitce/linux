...
void atc260x_get_bus_info(struct atc260x_dev *atc260x, uint *bus_num,
			  uint *bus_addr)
{
			...
			u16 mask, u16 val)
{
	...
	if (atc260x->ic_type == ATC260X_ICTYPE_2609A &&
	    reg == ATC2609A_PMU_SYS_CTL0 && (mask & 0x1) && (val & 0x1) == 0) {
		panic(KERN_ERR "GOT YOU, PMU_SYS_CTL0[0] set to 0!");
	}
	...
	case ATC260X_ACCESS_MODE_DIRECT:
		...
		if (ret < 0) {
			spin_unlock_irqrestore(&atc260x->dacc_spinlock,
					       irq_state_save);
			break;
		}
		...
		if (reg_tmp_val != reg_old_val) {
			ret =
			    atc260x->direct_write_reg(atc260x, reg,
						      reg_tmp_val);
		}
...
static int atc260x_pm_notifier_func(struct notifier_block *nb,
...
static void _atc260x_init_test_reg_access(struct atc260x_dev *atc260x)
{
	static const u16 sc_test_reg_tbl[ATC260X_ICTYPE_CNT][2] = {
		[ATC260X_ICTYPE_2603A] = {
		    ATC2603A_PMU_SYS_CTL8, ATC2603A_PMU_SYS_CTL9},
		[ATC260X_ICTYPE_2603C] = {
		    ATC2603C_PMU_FW_USE3, ATC2603C_PMU_FW_USE4},
		[ATC260X_ICTYPE_2609A] = {
		    ATC2609A_PMU_FW_USE0, ATC2609A_PMU_FW_USE1},
	};
	...
	for (i = 0; i < 10000; i++) {
		...
		if (ret) {
			dev_err(atc260x->dev,
				"test write err, i=%u reg=0x%x value=0x%x\n", i,
				ATC2603C_PMU_FW_USE4, tmp1);
			break;
		}
		...
		if (ret) {
			dev_err(atc260x->dev,
				"test write err, i=%u reg=0x%x value=0x%x\n", i,
				ATC2603C_PMU_FW_USE4, tmp2);
			break;
		}
		if (i == 5000) {
			dev_err(atc260x->dev,
				"test switch to direct_access mode\n");
			atc260x_set_reg_direct_access(atc260x, true);
		}
		ret = atc260x_reg_read(atc260x, reg1_addr);
		if (ret < 0 || (uint) ret != tmp1) {
			dev_err(atc260x->dev,
				"test write err, i=%u reg=0x%x value=0x%x org_value=0x%x\n",
...
static int _atc2603a_init_hardware(struct atc260x_dev *atc260x)
{
	...
	if ((ret & ~(1U << 15)) != 0x2aab) {
		dev_err(atc260x->dev,
			"Device is not a atc2603a: Register 0x%x value 0x%x!=0x%x\n",
...
static int _atc2603c_init_hardware(struct atc260x_dev *atc260x)
{
	...
	if (ret != 0x1bc0) {
		dev_err(atc260x->dev,
			"Device is not an atc2603c: Register 0x%x value 0x%x!=0x%x\n",
	...
	/* Output 32K clock */
	{
		...
		ret =
		    of_property_read_u32(atc260x->dev->of_node,
					 "losc_32k_output_enable", &of_cfg_val);
		if (ret == 0 && of_cfg_val != 0) {
			vsel_31 = 0;
			ret =
			    of_property_read_u32(atc260x->dev->of_node,
						 "losc_32k_output_voltage",
						 &of_cfg_val);
			if (ret == 0)
				vsel_31 = (of_cfg_val == 31) ? 1 : 0;

			atc260x_reg_setbits(atc260x, ATC2603C_PAD_VSEL,
					    (1U << 2), (vsel_31 << 2));
...
static int _atc2609a_init_hardware(struct atc260x_dev *atc260x)
{
	...
	if (ret != 0x0ff8) {
		dev_err(atc260x->dev,
			"Device is not an atc2609a: Register 0x%x value 0x%x!=0x%x\n",
	...
	/* Output 32K clock */
	{
		...
		ret =
		    of_property_read_u32(atc260x->dev->of_node,
					 "losc_32k_output_enable", &of_cfg_val);
		if (ret == 0 && of_cfg_val != 0) {
			atc260x_reg_setbits(atc260x, ATC2609A_PMU_SYS_CTL4,
					    (7U << 5), (4U << 5));
		}
	}
...
static const struct regmap_irq_chip *const
    sc_atc260x_irq_chip_tbl[ATC260X_ICTYPE_CNT] = {
...
#if 1
static ssize_t store_atc260x_reg(struct device *dev,
...
static ssize_t show_atc260x_auxadc_dbg_values(struct device *dev,
					      ...
					      char *buf)
{
	...
	for (channel = 0;; channel++) {
		...
		ch_unit_name =
		    atc260x_auxadc_channel_unit_name(atc260x, channel);
		if (ch_name == NULL || ch_unit_name == NULL) {
			break;
		}
		ret =
		    atc260x_auxadc_get_translated(atc260x, channel, &tr_value);
		if (ret == 0) {
			ret2 =
			    scnprintf(buf, PAGE_SIZE - size_accu,
				      "%-2u %-10s  %d %s\n", channel, ch_name,
...
static ssize_t show_atc260x_pstore_dbg_values(struct device *dev,
					      struct device_attribute *attr,
					      char *buf)
{
	struct atc260x_dev *atc260x;
...
int atc260x_core_dev_init(struct atc260x_dev *atc260x)
{
	...
	atc260x->reg_save_buf = devm_kzalloc(atc260x->dev,
					     sizeof(*(atc260x->reg_save_buf)) *
					     ATC260X_MAX_SAVE_REGS, GFP_KERNEL);
