...
static const struct atc260x_pstore_fmt sc_atc2603a_pstore_fmt_tbl[] = {
	...
	[ATC260X_PSTORE_TAG_RESUME_ADDR] = {
					    .name = "RESUME_ADDR",
					    .regs = {
					     ATC2603A_PMU_OC_STATUS,
					     ATC2603A_PMU_SYS_CTL3,
					     ATC2603A_PMU_SYS_CTL9,
					     ATC2603A_PMU_SYS_CTL8},
					    .reg_bm = {
					     (0xfU << 2), (0xfU << 6),
					     (0xff << 0), (0xffffU << 0)},
					    .src_bp = {28, 24, 16, 0},
...
static const struct atc260x_pstore_fmt *const
    sc_atc2603a_pstore_fmt_tbls[ATC260X_ICTYPE_CNT] = {
...
int atc260x_pstore_set(struct atc260x_dev *atc260x, uint tag, u32 value)
{
	...
	if (fmt->reg_bm[0] == 0 && value != 0) {
		dev_err(atc260x->dev,
			"%s(): pstore tag %s not available (ic_type=%u)\n",
	...
	for (i = 0; i < ARRAY_SIZE(fmt->regs); i++) {
		...
		ret =
		    atc260x_reg_setbits(atc260x, fmt->regs[i], reg_bm, reg_val);
		if (ret) {
			dev_err(atc260x->dev, "%s() io failed, ret=%d",
				__func__, ret);
			return ret;
		}
	}
...
int atc260x_pstore_dbg_dump(struct atc260x_dev *atc260x, char *buf,
			    uint bufsize)
{
	const struct atc260x_pstore_fmt *fmt;
