						    ...
						    unsigned int new_selector)
{
	 ...
	 *所以不能简单地比较selector. */
	old_volt =
	    (atc260x_regu->regulator_desc.ops->list_voltage) (rdev,
							      old_selector);
	if (old_volt < 0)
...
{
	static const struct atc260x_regu_hwinfo
	...
	static uint sc_atc260x_dcdc_hwinfo_tbl_len[ATC260X_ICTYPE_CNT] = {
		...
		[ATC260X_ICTYPE_2603C] =
		    ARRAY_SIZE(sc_atc2603c_ver_a_dcdc_hwinfo_tbl),
	...
	if ((ic_type == ATC260X_ICTYPE_2603C)
	    && (atc260x_get_ic_ver(atc260x) > 0)
	    && (atc260x_regu->regu_id == 2)) {
		init_data->constraints.min_uV = 1000000;
		sc_atc260x_dcdc_hwinfo_tbl_pool[ATC260X_ICTYPE_2603C] =
		    sc_atc2603c_ver_b_dcdc_hwinfo_tbl;
		sc_atc260x_dcdc_hwinfo_tbl_len[ATC260X_ICTYPE_2603C] =
		    ARRAY_SIZE(sc_atc2603c_ver_b_dcdc_hwinfo_tbl);
	}
	...
	if (hwinfo->volt_bands[1].volt_max_uv == 0) {
		...
		_atc260x_chk_constraints_vrang(&init_data->constraints,
					       hwinfo->
					       volt_bands[0].volt_min_uv,
					       hwinfo->
					       volt_bands[0].volt_max_uv, true);
