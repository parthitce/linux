...
/* convenient for dts adckey configuration*/
static int atc260x_adc_val = 0;
static ssize_t atc260x_adc_show( struct device *dev, struct device_attribute *attr, char *buf)
{
...
static int atc260x_adckeypad_config
	(struct atc260x_adckeypad_dev *atc260x_adckeypad)
{
	...
	ret = of_property_read_string(
		atc260x_adckeypad->dev->of_node,
			"adc_channel_name", &channel_name);
	if (ret) {
	...
	dev_info(atc260x_adckeypad->dev,
		"select AUXADC channel %s", channel_name);
...
static int atc260x_adckeypad_probe(struct platform_device *pdev)
{
...
#else
	/*get keymapsize*/
	ret = of_property_read_u32(np, "keymapsize",
	...
	ret = of_property_read_u32(np, "filter_dep",
	...
	/*get variance val */
	ret = of_property_read_u32(np, "variance",
	...
	if (ret) {
		dev_err(&pdev->dev,
			"Get right_adc_val failed ret = %d\r\n", ret);
		goto free_right;
	}
	...
	ret = of_property_read_u32(np,
		"poll_interval", &(poll_dev->poll_interval));
	...
	device_create_file(&input_dev->dev, &dev_attr_debug_adckey);
...
static int atc260x_adckeypad_remove(struct platform_device *pdev)
{
	...
	struct input_dev *input_dev = atc260x_adckeypad->poll_dev->input;

	device_remove_file(&input_dev->dev, &dev_attr_debug_adckey);
	platform_set_drvdata(pdev, NULL);
