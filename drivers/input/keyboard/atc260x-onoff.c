...
typedef int (*atc260x_onoff_init_func_t) (struct atc260x_onoff_dev *
					  atc260x_onoff);
...
struct atc260x_onoff_regdef {
	...
	u32 reg_int_ctl;
	u32 reg_int_pnd;
	u32 int_reg_w1c_bm;	/* ä¸€äº›"å†™1æ¸…0"åŠŸèƒ½ä½çš„ä½ç . ç”¨äºŽåº”ä»˜ä¸åŒçš„å†™å‰¯ä½œç”¨å¯„å­˜å™¨ä½æ··ç¼–çš„brain-damagedè®¾è®¡. */
	u32 pnd_reg_w1c_bm;	/* ä¸€äº›"å†™1æ¸…0"åŠŸèƒ½ä½çš„ä½ç . */
	u32 kdwn_state_bitm;
...
struct atc260x_hard_onoff {
	...
	unsigned char  poweroff; //ÊÇ·ñÅÐ¶Ï¹Ø»ú
};
static int hard_key= -1;    /*Ó²¼þ¿ª¹Ø×´Ì¬,0±íÊ¾¹Ø»ú×´Ì¬£¬1±íÊ¾¿ª»ú×´Ì¬,-1±íÊ¾²»ÅäÖÃ*/
...
static int _atc2603a_onoff_init(struct atc260x_onoff_dev *atc260x_onoff)
{
	...
	return atc260x_reg_setbits(atc260x_onoff->atc260x,
				   ATC2603A_PMU_SYS_CTL2,
				   (1U << 12) | (3U << 10) | (1 << 14) | (1 <<
									  13),
				   (1U << 12) | (onoff_time << 10) | (1 << 14) |
				   (1 << 13));
...
static const struct atc260x_onoff_regdef *const
    sc_atc260x_onoff_regdef_tbl[ATC260X_ICTYPE_CNT] = {
...
static void _atc260x_onoff_w1clr_pndreg(struct atc260x_dev *atc260x,
					const struct atc260x_onoff_regdef
					*regdef, uint mask)
{
	int ret;
...
static void atc260x_poll_onoff(struct work_struct *work)
{
	...
	if (ret >= 0) {
		poll = atc260x_onoff->key_trigger
		    || (ret & regdef->kdwn_state_bitm);
...
static void _atc260x_onoff_trigger_key_process(struct atc260x_onoff_dev
					       *atc260x_onoff, uint delay)
{
	struct atc260x_dev *atc260x = atc260x_onoff->atc260x;
	const struct atc260x_onoff_regdef *regdef = atc260x_onoff->regdef;
...
static int atc260x_onoff_suspend(struct device *dev)
{
	...
	/* < 0.5s for short_key, > 0.5s for long_key */
	atc260x_reg_setbits(atc260x_onoff->atc260x, ATC2603A_PMU_SYS_CTL2,
			    (3U << 10), (0 << 10));
...
static void atc260x_onoff_resume_complete(struct device *dev)
{
	...
	if (atc260x_onoff->suspend_state != 0) {
		...
		if (wakeup_flag &
		    (OWL_PMIC_WAKEUP_SRC_ONOFF_SHORT | sim_wake_srcs)) {
			dev_info(atc260x_onoff->dev,
				 "resend/simulate On/Off short press...\n");
			...
			dev_info(atc260x_onoff->dev,
				 "resend/simulate On/Off long press...\n");
			input_report_key(atc260x_onoff->idev, KEY_POWER, 1);
			input_sync(atc260x_onoff->idev);
			_atc260x_onoff_trigger_key_process(atc260x_onoff,
							   msecs_to_jiffies
							   (2500));
		}
#else
		...
		if (wakeup_flag &
		    (OWL_PMIC_WAKEUP_SRC_ONOFF_SHORT |
		     OWL_PMIC_WAKEUP_SRC_ONOFF_LONG | sim_wake_srcs)) {
			dev_info(atc260x_onoff->dev,
				 "resend/simulate On/Off short press...\n");
			input_report_key(atc260x_onoff->idev, KEY_POWER, 1);
			input_report_key(atc260x_onoff->idev, KEY_POWER, 0);
...
static int atc260x_onoff_probe(struct platform_device *pdev)
{
	...
	atc260x_onoff =
	    devm_kzalloc(&pdev->dev, sizeof(struct atc260x_onoff_dev),
			 GFP_KERNEL);
	if (!atc260x_onoff) {
	...
	ret = devm_request_threaded_irq(&pdev->dev, atc260x_onoff->irq, NULL,
					atc260x_onoff_irq, IRQF_TRIGGER_HIGH,
					"atc260x_onoff", atc260x_onoff);
	if (ret < 0) {
