...
#define TP_IRQ_PORT	  		OWL_EXT_IRQ_SIRQ1
#define GPIO_IN 25 
...
//        printk("gslX680 Interrupt success\n");     
...
static int gslX680_ts_init(struct i2c_client *client, struct gsl_ts *ts)
{
	...
	set_bit(ABS_X, input_device->absbit);  //by lyc
	set_bit(ABS_Y, input_device->absbit);  //by lyc
	set_bit(ABS_PRESSURE, input_device->absbit); //by lyc 
...
//	input_set_abs_params(input_device, BTN_TOUCH, 0, 255, 0, 0);
//        input_set_abs_params(input_device, ABS_PRESSURE, 0, 1, 0, 0);

#if CFG_TP_USE_CONFIG
	if(cfg_dts.rotate == 90 || cfg_dts.rotate == 270)
	{
		input_set_abs_params(input_device, ABS_Y, 0, cfg_dts.xMax, 0, 0);
		input_set_abs_params(input_device, ABS_X, 0, cfg_dts.yMax, 0, 0);
	}
	else{
		input_set_abs_params(input_device, ABS_X, 0, cfg_dts.xMax, 0, 0);
		input_set_abs_params(input_device, ABS_Y, 0, cfg_dts.yMax, 0, 0);
	}
#else
    input_set_abs_params(input_device, ABS_X, 0, TP_MAX_X, 0, 0);
    input_set_abs_params(input_device, ABS_Y, 0, TP_MAX_Y, 0, 0);
#endif
...
//	rc=  request_irq(client->irq, gsl_ts_irq, IRQF_TRIGGER_RISING | IRQF_DISABLED, client->name, ts);
      rc=  request_irq(gpio_to_irq(GPIO_IN), gsl_ts_irq, IRQF_TRIGGER_RISING | IRQF_DISABLED, client->name, ts);
	if (rc < 0) {
...
error_req_irq_fail:
//	free_irq(ts->irq, ts);
	free_irq(gpio_to_irq(GPIO_IN), ts);
        ...
        if ((ret = gpio_request(GPIO_IN, THIS_MODULE->name)) !=0)
         {
            ...
            printk("request GPIO_IN  success");
       }
        ...
        if ((ret = gpio_direction_input(GPIO_IN)) !=0)
         {
            ...
            printk("request gpio_direction_input success");
       }
#if 0
#if CFG_TP_USE_CONFIG
