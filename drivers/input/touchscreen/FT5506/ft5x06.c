...
#define TIME_OF_DISCARD         (msecs_to_jiffies(2000))
#define PROTOCOL_B
enum FT5X06_MODE {
	INVALID_MODE ,
	OPER_MODE ,
...
FTS_BOOL cmd_write(struct i2c_client *client, FTS_BYTE btcmd,
		FTS_BYTE btPara1, FTS_BYTE btPara2,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret || (err_code == 0x03 ||
		err_code == 0x05 || err_code == 0x1a)) {
		ft5x06_reset(ftdevice);
	}
...
static int tp_of_data_get(void)
{
	...
	if (of_node == NULL) {
		printk(KERN_ERR"%s,%d,find the ft5x06 dts err!\n", __func__, __LINE__);
		return -1;
	}
