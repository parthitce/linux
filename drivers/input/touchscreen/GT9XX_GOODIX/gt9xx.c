...
#include <linux/irq.h>		//含linux中断处理函数
#include "gt9xx.h"		
...
static const char *goodix_ts_name = "goodix-ts"; 	//创建一个goodix设备名字
static const char *goodix_input_phys = "input/ts";	//定义为input子设备
static struct workqueue_struct *goodix_wq;		//定义一个goodix工作队列
struct i2c_client * i2c_connect_client = NULL;		//定义一个i2c子设备为null
int gtp_rst_gpio;
int gtp_int_gpio;
...
s32 gtp_send_cfg(struct i2c_client *client)
{
    ...
    for(retry = 0;retry < 188; retry++){
	if( (retry >1 && (retry-2)%11 ==0) || retry == 2)
		printk("\n");
    ...
    for (retry = 0; retry < 5; retry++)
    {
        ...
        ret = gtp_i2c_write(client, config , 186 + 2);
        if (ret > 0)
        {
            break;
        }
...
#endif
    GTP_INFO("gtp_send_cfg : send ret = %d.\n",ret);
    return ret;
}
/*******************************************************
Function:
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
    ...
    GTP_DEBUG("ID:%d, X:%d, Y:%d, W:%d,X_MAX=%d,Y_MAX=%d;\n", id, x, y, w ,ts->abs_x_max,ts->abs_y_max);
}
...
#endif
    printk("pen_init: ts->abs_x_max=%d,ts->abs_y_max=%d;\n",ts->abs_x_max,ts->abs_y_max);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//gtp_rst_gpio = GTP_RST_PORT;  //init in other place.
	//gtp_int_gpio = GTP_INT_PORT;  //init in other place.
#endif
...
static int tp_of_data_get(void)
{
        ...
        of_node = of_find_compatible_node(NULL, NULL, COMPATIBLE_NAME);
        if (of_node == NULL) {
                printk(KERN_ERR"%s,%d,find the gt9xx dts err!\n", __func__, __LINE__);
                return -1;
        }else{
		printk("[%s]:success find of_node=0x%p",__func__,of_node);
	}
	...
	printk("use irq_of_parse_and_map cfg_dts.sirq=%d.\n",cfg_dts.sirq);
	ret = of_property_read_u32(of_node, "int_gpios", &gtp_int_gpio);
	if (ret) {
                printk(" failed to get int_gpios num\n");
                goto fail;
        }
	printk("use of_property_read_u32 get  gtp_int_gpio=%d.\n",gtp_int_gpio);
	//gtp_int_gpio = cfg_dts.sirq;
	//gtp_int_gpio = 24; //INT --GPIOA24 //Jursery 20170326
        ...
        gtp_rst_gpio = of_get_named_gpio_flags(of_node, "reset_gpios", 0, &flags);
	...
	printk("Ready_print param.\n");
        printk("gpio reset num:%d, gpio int num:%d,  i2c_addr:%02x, irq_number:%d,x_pixel:%d, y_pixel:%d,x_revert_en:%d,y_revert_en:%d,xy_swap_en:%d, max_point:%d, rotate:%d, i2cNum:%d\n",
                gtp_rst_gpio,
                gtp_int_gpio,
                cfg_dts.i2cAddr,
                cfg_dts.sirq,
...
static int __init goodix_ts_init(void)
{
    ...
    err = tp_of_data_get();
    if (err < 0) {
	printk("gt9xx driver get config from dts file err!!!\n");
	return err;
    }
