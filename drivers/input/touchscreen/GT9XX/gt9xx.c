...
static struct gt9xx_cfg_dts cfg_dts;
static unsigned gpio_reset;
#define GT9XX_I2C_ADAPTER      (4)
...
/* 	struct i2c_msg msgs;
	...
	msgs.len   = GTP_ADDR_LENGTH;
	msgs.buf   = &buf[0];
	//msgs[0].scl_rate = 300 * 1000;    // for Rockchip, etc.

	while(retries < 5) {
		//GTP_INFO("i2c_transfer...");
	...
	msgs.flags = I2C_M_RD;
	msgs.addr  = client->addr;
	msgs.len   = len - GTP_ADDR_LENGTH;
	msgs.buf   = &buf[GTP_ADDR_LENGTH];
	//msgs[1].scl_rate = 300 * 1000;
...
void gtp_int_sync(s32 ms)
{
	/*GTP_GPIO_OUTPUT(cfg_dts.sirq, 0);*/
	msleep(ms);
	/*GTP_GPIO_AS_INT(cfg_dts.sirq);*/
}
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	/*GTP_GPIO_OUTPUT(cfg_dts.sirq, client->addr == 0x5d);  //0x14 */
	...
	/*GTP_GPIO_AS_INPUT(gpio_reset);   end select I2C slave addr*/
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data *ts)
{
	...
	if (CHIP_TYPE_GT9F == ts->chip_type) {
		...
		/*GTP_GPIO_OUTPUT(cfg_dts.sirq, 1);*/
		msleep(5);
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	GTP_DEBUG_FUNC();
/* 	GTP_INFO("cfg_dts.sirq %d...",cfg_dts.sirq);
	ret = GTP_GPIO_REQUEST(cfg_dts.sirq, "GTP_INT_IRQ");
	if (ret < 0)
	{
		GTP_INFO("cfg_dts.sirq error %d...",cfg_dts.sirq);
		GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d", (s32)cfg_dts.sirq, ret);
		ret = -ENODEV;
	}
	else
	{
		...
		GTP_INFO("ts->client->irq = %d...",cfg_dts.sirq);
	} */

	ts->client->irq = cfg_dts.sirq;/* //cfg_dts.sirq; */
	GTP_INFO("ts->client->irq %d...", cfg_dts.sirq);
	GTP_INFO("gpio_reset %d...", gpio_reset);
/* 	ret = GTP_GPIO_REQUEST(gpio_reset, "gpio_reset"); */
	gpio_request(gpio_reset, GTP_I2C_NAME);
	if (ret < 0) {
		GTP_INFO("gpio_reset error %d...", gpio_reset);
		GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d", gpio_reset, ret);
		ret = -ENODEV;
	}

/* 	GTP_GPIO_AS_INPUT(gpio_reset); */

	gtp_reset_guitar(ts->client, 20);

	if (ret < 0) {
		GTP_GPIO_FREE(gpio_reset);
		/*GTP_GPIO_FREE(cfg_dts.sirq);*/
	}
...
/* 		GTP_GPIO_AS_INPUT(cfg_dts.sirq);
		GTP_GPIO_FREE(cfg_dts.sirq); */
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	if (sysfs_create_group(&ts->input_dev->dev.kobj, &tp_attr_group) < 0)
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/*ret =  request_irq(ts->client->irq, goodix_ts_irq_handler, IRQF_TRIGGER_HIGH | IRQF_DISABLED, ts->client->name, ts);*/
	if (ret < 0) {
...
/* 			GTP_GPIO_AS_INPUT(cfg_dts.sirq);
	...
	sysfs_remove_group(&ts->input_dev->dev.kobj, &tp_attr_group);
...
{
	GTP_INFO("----[GT9XX] Enter tp_suspend---");
	struct goodix_ts_data *ts = i2c_get_clientdata(client);
	disable_irq_nosync(ts->client->irq);
	GTP_INFO("---disable_irq %d----", ts->client->irq);
	flush_workqueue(goodix_wq);
	flush_work(&ts->work);

	GTP_GPIO_OUTPUT(gpio_reset, 0);

	if (tp_regulator) {
		 current_val = regulator_get_voltage(tp_regulator);
		 regulator_disable(tp_regulator);
		 GTP_INFO("Nova disable regulator %d", current_val);
	}
...
{
	int ret;
	GTP_INFO("----[GT9XX] Enter tp_resume-----");
	struct goodix_ts_data *ts = i2c_get_clientdata(client);

/* 	if(true == this_ts->is_suspended)
	{
		printk(" power is turned on!\n");
	} */

	if (tp_regulator) {
	    regulator_set_voltage(tp_regulator, cfg_dts.vol_min, cfg_dts.vol_max);
	    ret = regulator_enable(tp_regulator);
	}

	GTP_GPIO_OUTPUT(gpio_reset, 1);
	enable_irq(ts->client->irq);
	GTP_INFO("---enable_irq %d----", ts->client->irq);
...
static int tp_of_data_get(void)
{
	...
	of_node = of_find_compatible_node(NULL, NULL, "gt9xx");
	if (of_node == NULL) {
		printk(KERN_ERR"%s,%d,find the gt9xx dts err!\n", __func__, __LINE__);
		return -1;
	}
	...
	cfg_dts.i2cNum = GT9XX_I2C_ADAPTER;
...
static struct of_device_id gt9xx_of_match[] = {
	   { .compatible = "gt9xx" },
	   { }
};
...
static struct i2c_driver goodix_ts_driver = {
	...
	.driver = {
		.name     = GTP_I2C_NAME,
		.owner    = THIS_MODULE,
		.of_match_table = of_match_ptr(gt9xx_of_match),
	},
};
...
static int goodix_ts_init(void)
{
	...
	err = tp_of_data_get();
	if (err < 0) {
		printk("gt9xx get config err!!!");
		return err;
	}

	/*tp_regulator has enabled in the follow function*/
	tp_regulator = regulator_init(cfg_dts.regulator,
	   cfg_dts.vol_min, cfg_dts.vol_max);
	if (!tp_regulator) {
		GTP_INFO("tp_regulator failed...");
