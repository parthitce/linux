...
int read_storage(void *buf, int start, int size)
{
	...
	for (i = 0; i < 6; i++) {
		...
		if (IS_ERR(file) || file->f_op == NULL
			...
			if (IS_ERR(file)) {
				PRINT_DBG("file is err\n");
			}
			else if (file->f_op == NULL) {
				PRINT_DBG("file->f_op == NULL\n");
			}
...
int read_mi_head(misc_info_head_t *head)
{
	...
	if (tmp_head->magic != MISC_INFO_MAGIC || tmp_head->length > MISC_INFO_MAX_SIZE
		...
		ret = 0;/*because of historical issues,this must set ret = 0*/
		PRINT_ERR("%s, line %d, magic 0x%x, length %d, item num %d, ret = %d\n",
			__FUNCTION__, __LINE__,
			tmp_head->magic, tmp_head->length, tmp_head->item_num, ret);
		goto OUT_FAILED;
	}
 ...
 *
 * 1.ret < 0  read fail
 * 2.ret > 0  read success, ret is read value length
 *
 */
int read_mi_item(char *name, void *buf, unsigned int count)
{
	...
	if (strlen(name) > sizeof(head->item_head[0].name)) {
		ret = -1;
		PRINT_ERR("invalid name, too large, ret = %d\n", ret);
		goto OUT_FAILED;
	}
	for (item = 0; item < head->item_num; item++) {
		len = strlen(name) > strlen(head->item_head[item].name) ? strlen(name) : strlen(head->item_head[item].name);
		if (memcmp(name, head->item_head[item].name, len) == 0)
			break;
	}
	if (item == head->item_num) {
		ret = -1;
		PRINT_ERR("can not find %s, ret = %d\n", name, ret);
		goto OUT_FAILED;
	}
 ...
 *
 * 1.ret < 0  write fail
 * 2.ret >= 0 write success, ret is write value length
 *
 */
int write_mi_item(char *name, void *buf, unsigned int count)
{
	...
	ret = ret - sizeof(int) - sizeof(packet_item_t);
OUT_FAILED:
	if (packet) {
		FREE(packet);
		packet = NULL;
...
int get_item_size(misc_info_head_t *head, char *name)
{
	...
	for (item = 0; item < head->item_num; item++) {
		len = strlen(name) > strlen(head->item_head[item].name) ? strlen(name) : strlen(head->item_head[item].name);
		if (memcmp(head->item_head[item].name, name, len) == 0)
	...
	ioctl_item_t __user *item)
{
	...
	//PRINT_DBG("%s[%d], &item32->name = %p, &item32->size = %p, &item32->data = %p\n",
	//	__FUNCTION__, __LINE__, &item32->name, &item32->size, &item32->data);
	//PRINT_DBG("%s[%d], &item32->name = %x, &item32->size = %x, &item32->data = %x\n",
	//	__FUNCTION__, __LINE__, item32->name, item32->size, item32->data);

	err = copy_in_user(item->name, item32->name, sizeof(item32->name));
	err |= get_user(i, &item32->size);
	err |= put_user(i, &item->size);
	err |= get_user(datap, &item32->data);
	err |= put_user(compat_ptr(datap), &item->data);

	//PRINT_DBG("%s[%d], &item->name = %p, &item->size = %p, &item->data = %p\n",
	//	__FUNCTION__, __LINE__, &item->name, &item->size, &item->data);
	//PRINT_DBG("%s[%d], &item->name = %x, &item->size = %x, &item->data = %x\n",
	//	__FUNCTION__, __LINE__, item->name, item->size, item->data);

	return err;
}

static long mi_ioctl_compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int ret;
	ioctl_item_t32 __user *item32;
	ioctl_item_t *item;
	int err;
	PRINT_DBG("%s[%d], cmd = 0x%x, arg = 0x%x\n",__FUNCTION__, __LINE__, cmd, arg);

	switch (cmd) {
	case GET_ITEM_DATA:
	case SET_ITEM_DATA:
		item32 = compat_ptr(arg);
		item = compat_alloc_user_space(sizeof(*item));
		if (item == NULL) {
			return -EFAULT;
		}

		ret = compat_get_misc_trans(item32, item);
		if (ret) {
			return -EFAULT;
		}

		return mi_ioctl(filp, cmd, item);
	case FORMAT_MISC_INFO:
		ret = mi_ioctl(filp, cmd, (unsigned long)compat_ptr(arg));
		return ret;
	}
}
