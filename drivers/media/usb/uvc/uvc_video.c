		...
		struct uvc_buffer *buf, const __u8 *data, int len)
{
	...
	if (data[1] & UVC_STREAM_ERR) {
		uvc_trace(UVC_TRACE_FRAME, "Marking buffer as bad (error bit "
			  "set).\n");
		/*added for one frame split to two parts, or one frame with
                 * err data problems, ActionsCode(author:liyuan, change_code)*/	  
	...
	struct uvc_buffer *buf)
{
	...
	for (i = 0; i < urb->number_of_packets; ++i) {
		if (urb->iso_frame_desc[i].status < 0) {
			...
			if (buf != NULL) {
			        /*added for one frame split to two parts, or
                                 *one frame with err data problems,
		...
		if (buf->state == UVC_BUF_STATE_READY) {
			if (buf->length != buf->bytesused &&
				...
				/*added for one frame split to two parts,
                                 *or one frame with err data problems,
                                 ActionsCode(author:liyuan, change_code)*/  
				buf->error = UVC_BUF_ERR_NOTFULL;
			}
...
#ifdef CONFIG_ASOC_CAMERA
			dma_free_coherent(uvc_misc_device.this_device,
				 stream->urb_size, stream->urb_buffer[i],
					stream->urb_dma[i]);
#else
...
#ifndef CONFIG_DMA_NONCOHERENT
#ifdef CONFIG_ASOC_CAMERA
			stream->urb_buffer[i] = dma_alloc_coherent(
				uvc_misc_device.this_device, stream->urb_size,
