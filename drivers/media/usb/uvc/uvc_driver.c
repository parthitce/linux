...
/*added for one frame split to two parts, or one frame with err data problems,
...
static void get_dropframesnum(struct uvc_streaming *stream){
	...
	if(i>=0 && i<len){
		stream->uvc_drop_nframes =
                        dev_dropframes[i].drop_nframes_needed;
	}else{
...
struct miscdevice  uvc_misc_device = {
				.name = "uvc",
...
#ifdef CONFIG_ASOC_CAMERA
	ret = misc_register(&uvc_misc_device);
	if (ret)
		return ret;
	uvc_misc_device.this_device->coherent_dma_mask = DMA_BIT_MASK(32);
	uvc_misc_device.this_device->dma_mask =
			&(uvc_misc_device.this_device->coherent_dma_mask);
#endif
...
{
#ifdef CONFIG_ASOC_CAMERA
	misc_deregister(&uvc_misc_device);
#endif
