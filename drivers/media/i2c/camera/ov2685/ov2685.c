	...
	unsigned int data_width, unsigned int reg, unsigned int *dest)
{
	...
	if (ret < 0) {
		pr_info("write register %s error %d",
	...
	if (ret >= 0) {
		...
		pr_info("read register %s error %d",
			CAMERA_MODULE_NAME, ret);

	return ret;
}
static int camera_i2c_write(struct i2c_adapter *i2c_adap,
	unsigned int data_width, unsigned int reg, unsigned int data)
{
	unsigned char regs_array[4] = {0, 0, 0, 0};
	unsigned char data_array[4] = {0, 0, 0, 0};
...
static int module_set_colorfx(struct v4l2_subdev *sd,
	struct v4l2_ctrl *ctrl)
{
	return 0;
}
...
static int  module_set_af_region(struct v4l2_subdev *sd,
	struct v4l2_ctrl *ctrl)
{
	return 0;
}
...
static int module_s_mirror_flip(struct v4l2_subdev *sd,
	struct v4l2_ctrl *ctrl)
{
	int ret = 0;
	return ret;
...
static int  module_set_af_mode(struct v4l2_subdev *sd,
	struct v4l2_ctrl *ctrl)
{
	return 0;
}

static int module_get_af_mode(struct v4l2_subdev *sd,
	struct v4l2_ctrl *ctrl)
{
	int ret = 0;
	ctrl->val = NONE_AF;
