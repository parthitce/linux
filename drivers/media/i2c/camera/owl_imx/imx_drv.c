...
#define imx_write(val, reg) writel_relaxed(val, imx_dev->base + (reg - IMX_BASE))
...
#define sps_write(val, reg) writel_relaxed(val, imx_sps_base + (reg - SPS_PG_BASE))
...
#define DBUG_I(fmt, args...) pr_info("AIMX, %s %d, " fmt, __func__, __LINE__, ##args)
#define DBUG_E(fmt, args...) pr_err("AIMX, %s %d, " fmt, __func__, __LINE__, ##args)
	...
	case IMX_IOC_START:
		{
			...
			if (unlikely(0 == time_left)) {
				DBUG_E
				    ("err! timeout when enabling running! IMX_CTRL:0x%x\n",
				     status);
				imx_reset();
	...
	case IMX_IOC_QUERY:
		{
			...
			if (unlikely(0 == time_left)) {
				DBUG_E
				    ("err! timeout when querying! IMX_CTRL:0x%x\n",
				     imx_read(IMX_CTRL));
				imx_irq_disable();
				imx_reset();
				mutex_unlock(&imx_ioctl_mutex);
...
static int __init owl_imx_probe(struct platform_device *pdev)
{
	...
	/*
	   ...
	   if (ret) {
	   dev_err(imx_dev->dev, "err! failed to read imx-setting, err %d\n", ret);
	   return ret;
	   }
	   }
	   else {
