				...
				struct soc_camera_device *icd)
{
	...
	module_camera_clock_enable(cdev, cam_param);
...
static inline void set_preline(struct soc_camera_device *icd)
{
    ...
    state = reg_read(GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE);
    state &= ~CH_PRELINE_NUM_MASK;
    state |= CH_PRELINE_NUM(PRELINE_NUM);
	reg_write(state, GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE);
}
...
static inline int set_row_range(struct soc_camera_device *icd)
{
	...
	if ((height > MODULE_MAX_HEIGHT) || (height < 1) || \
            (start >= MODULE_MAX_HEIGHT) || \
            (end >= MODULE_MAX_HEIGHT)) {
		return -EINVAL;
	}

    reg_write(CH_ROW_START(0) | CH_ROW_END(end - start),
    	  GMODULEMAPADDR, SI_ROW_RANGE, MODULE_BASE);
...
{
	struct camera_param *cam_param = icd->host_priv;
	unsigned int module_ctl = reg_read(GMODULEMAPADDR, SI_FORMAT,
					   MODULE_BASE);

	module_ctl &= ~(MODULE_CTL_CHANNEL_COLOR_SEQ_MASK |
			MODULE_CTL_CHANNEL_RAW_CLOLR_SEQ_MASK);
	switch (code) {
	case V4L2_MBUS_FMT_UYVY8_2X8:	/* UYVY */
		module_ctl |= MODULE_CTL_CHANNEL_COLOR_SEQ_UYVY;
		break;
	case V4L2_MBUS_FMT_VYUY8_2X8:	/* VYUY */
		module_ctl |= MODULE_CTL_CHANNEL_COLOR_SEQ_VYUY;
		break;
	case V4L2_MBUS_FMT_YUYV8_2X8:	/* YUYV */
		module_ctl |= MODULE_CTL_CHANNEL_COLOR_SEQ_YUYV;
		break;
	case V4L2_MBUS_FMT_YVYU8_2X8:	/* YVYU */
		module_ctl |= MODULE_CTL_CHANNEL_COLOR_SEQ_YVYU;
		break;
	default:
    ...
    module_ctl &= ~MODULE_CTL_CHANNEL_RAW_CLOLR_SEQ_MASK;
	reg_write(module_ctl, GMODULEMAPADDR, SI_FORMAT, MODULE_BASE);
...
static inline void get_fmt(u32 fourcc, unsigned int *fmt)
{
	...
	case V4L2_PIX_FMT_NV12:	/*420 semi-planar */
		*fmt |= MODULE_OUT_FMT_NV12;
		break;
	case V4L2_PIX_FMT_NV21:	/*420 interleaved semi-planar */
		*fmt |= MODULE_OUT_FMT_NV21;
		break;
	default:		/* Raw RGB */
		break;
			       ...
			       unsigned int common_flags)
{
	...
	module_ch_ctl = reg_read(GMODULEMAPADDR, SI_SYNC_PRIORITY, MODULE_BASE);

	if (common_flags & V4L2_MBUS_HSYNC_ACTIVE_LOW) {
		module_ch_ctl &=
		    ~MODULE_CTL_CHANNEL_HSYNC_ACTIVE_HIGH;
	} else {
		module_ch_ctl |= MODULE_CTL_CHANNEL_HSYNC_ACTIVE_HIGH;
	}

	if (common_flags & V4L2_MBUS_VSYNC_ACTIVE_LOW) {
		module_ch_ctl &=
		    ~MODULE_CTL_CHANNEL_VSYNC_ACTIVE_HIGH;
	} else {
		module_ch_ctl |= MODULE_CTL_CHANNEL_VSYNC_ACTIVE_HIGH;
	}
	reg_write(module_ch_ctl, GMODULEMAPADDR, SI_SYNC_PRIORITY, MODULE_BASE);
			  ...
			  struct soc_camera_device *icd)
{
	...
	unsigned int module_enable, module_int_stat, module_ctl, module_polarity;
	...
	module_ctl = reg_read(GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE);
	module_ctl |= MODULE_INT_STAT_CH_PL_INT_EN;
	reg_write(module_ctl, GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE);
    module_polarity = reg_read(GMODULEMAPADDR, SI_SYNC_PRIORITY, MODULE_BASE);
	/* vsync: */
	module_polarity = (module_polarity & (~(0x1 << 1))) |
	    (module_setting->hs_pol << 1);
	/* hsync: */
	module_polarity = (module_polarity & (~(0x1 << 0))) |
	    (module_setting->vs_pol << 0);
	reg_write(module_polarity, GMODULEMAPADDR, SI_SYNC_PRIORITY, MODULE_BASE);

	cam_param->started = DEV_START;

	module_ctl = reg_read(GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE);
	module_ctl |= MODULE_ENABLE_EN;
	reg_write(module_ctl, GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE);
...
static void clear_isr(struct camera_param *cam_param)
{
	...
	int_stat = reg_read(GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE);
	reg_write(int_stat, GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE);
}
			 ...
			 struct soc_camera_device *icd)
{
	...
	module_enable = reg_read(GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE);

	reg_write(module_enable & (~MODULE_ENABLE_EN), GMODULEMAPADDR,
		  SI_CON_STAT, MODULE_BASE);

	/* enable frame end interrupt to check that frame tx already finish */
	int_stat = reg_read(GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE);

	m_isr_init |= MODULE_INT_STAT_CH_PL_INT_EN;
	int_stat &= (~m_isr_init);
	int_stat |= MODULE_INT_STAT_CH_FRAME_END_INT_EN;
	reg_write(int_stat, GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE);
...
static irqreturn_t camera_host_isr(int irq, void *data)
{
	...
	for (i = 0; i < 2; i++) {
		if (cam_dev->icds[i]) {
			...
			module_int_stat = reg_read(GMODULEMAPADDR, SI_CON_STAT,
						   MODULE_BASE);
...
static int get_supported_mbus_param(struct camera_param *cam_param)
{
	...
	/*
	 * ats3605  camera interface only works in master mode
...
static void camera_remove_device(struct soc_camera_device *icd)
{
	...
	reg_write(reg_read(GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE),
...
static int camera_resume(struct device *dev)
{
    ...
    /*if bit8--bit11 is 1, rewrite to clear*/
	reg_write(reg_read(GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE),
 		  GMODULEMAPADDR, SI_CON_STAT, MODULE_BASE);
...
static int camera_host_probe(struct platform_device *pdev)
{
	...
	if (err) {
		dev_err(&pdev->dev, "Unable to register si %d interrupt.\n",
			cam_dev->irq);
		err = -EBUSY;
		goto egetirq;
	}
...
static int camera_shutdown(struct device *dev)
{
	struct soc_camera_host *ici = to_soc_camera_host(dev);
	struct camera_dev *cam_dev = container_of(ici, struct camera_dev,
						  soc_host);
	module_gpio_exit(&cam_dev->spinfo);
	return 0;
}
...
static int __init camera_init(void)
{
	...
	if (ret)
		module_err(":Could not register legacy si driver\n");
