					 ...
					 struct v4l2_ctrl *ctrl)
{
	...
	switch(auto_white_balance)
	{
		...
		case 1:	
			ret = camera_write_array(client->adapter, module_whitebance_auto_regs);
			break;
						...
						struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_WHITE_BALANCE_INCANDESCENT: /* 白炽光 */
		ret = camera_write_array(client->adapter, module_whitebance_incandescent_regs);
		break;
	
	case V4L2_WHITE_BALANCE_FLUORESCENT: /* 荧光灯 */
		ret = camera_write_array(client->adapter, module_whitebance_fluorescent_regs);
		break;
	
	case V4L2_WHITE_BALANCE_DAYLIGHT: /* 日光 (晴天)*/
		ret = camera_write_array(client->adapter, module_whitebance_sunny_regs);
		break;
	
	//case V4L2_WHITE_BALANCE_CLOUDY: /* 多云 （阴天）*/
	//	ret = camera_write_array(client->adapter, module_whitebance_cloudy_regs);
	//	break;
...
void sensor_power_on(bool rear, struct sensor_pwd_info *spinfo, bool hardware)
{
	if (hardware) {
		if (rear) {
			...
			DBG_INFO("Cold power on gc2145 as front camera.\n");
		}
	} else {
		if (rear) {
			set_gpio_level(&spinfo->gpio_rear, GPIO_HIGH);
			...
			DBG_INFO("Soft power on gc2145 as front camera.\n");
		}
	}
}
...
void sensor_power_off(bool rear, struct sensor_pwd_info *spinfo, bool hardware)
{
	if (hardware) {
		if (rear) {
			...
			DBG_INFO("Cold power off gc2145 as front camera.\n");
		}
	} else {
		if (rear)
			set_gpio_level(&spinfo->gpio_rear, GPIO_HIGH);
		else
			DBG_INFO("Soft power off gc2145 as front camera.\n");
	}
}
