...
struct isp_regulators    g_isp_ir;
	...
	const char *property, const char *stem)
{
	...
	if (NULL == pnode) {
		pr_err("err: fail to get node[%s]", property);
		return value;
	}
...
static int init_common(void)
{
	...
	g__spinfo.ch_clk[ISP_CHANNEL_0] = NULL;
	g__spinfo.ch_clk[ISP_CHANNEL_1] = NULL;
	...
	struct isp_regulators *ir)
{
	...
	DBG_INFO("");
	if (!gpio_init(fdt_node, "dvdd-gpios", dvdd_gpio, GPIO_LOW))/* poweroff */
		ir->dvdd_use_gpio = 1;
	else
		ir->dvdd_use_gpio = 0;
	...
	if (!strcmp(avdd_src, "regulator")) {
		...
		if (gpio_init(fdt_node, "avdd-gpios", gpio, GPIO_LOW))/* poweroff */
			goto fail;
...
static void isp_regulator_exit_power_off(struct isp_regulators *ir)
{
	DBG_INFO("");
	if (ir->dvdd_use_gpio)
...
static void isp_regulator_exit_power_hold(struct isp_regulators *ir)
{
	DBG_INFO("");
	if (ir->dvdd_use_gpio)
...
static int isp_gpio_init(struct device_node *fdt_node, struct sensor_pwd_info *spinfo)
{
	...
	if (!strcmp(board_type, "ces")) {
		...
		spinfo->gpio_rear_reset.active_level = spinfo->gpio_front_reset.active_level;
		board_type_num = 0;
	} else {
	...
	if (!strcmp(sensors, "front")) {
		...
		if (gpio_init(fdt_node, "pwdn-front-gpios", &spinfo->gpio_front, GPIO_LOW))
			goto free_reset;
		if (gpio_init(fdt_node, "pwdn-rear-gpios", &spinfo->gpio_rear, GPIO_LOW))
			goto free_reset;
...
static int isp_clk_enable(void)
{
	...
	if (g_isp_clk != NULL) {
		...
		/*set the rate of the isp */
		ret = clk_set_rate(g_isp_clk, ISP_MODULE_CLOCK);
	}
	return ret;
}
...
int detect_init(void)
{
	...
	if (NULL == fdt_node) {
		DBG_ERR("err: no [flashlight] in dts\n");
