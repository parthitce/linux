...
#include "./../owl_host/owl_isp.h"
#include "./../owl_host/isp_pwd-owl.h"
#include "./../flashlight/flashlight.h"

/*#define GL5209_EVB_DDR_V01_V02*/
bool light_on;
extern struct sensor_pwd_info   g__spinfo;
extern struct isp_regulators    g_isp_ir;
...
static int camera_module_get_params(struct i2c_client *client, u32 *width, u32 *height)
{
...
static int camera_module_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_POWER_LINE_FREQUENCY:
		...
		break;
	default:
		DBG_ERR("v4l2-ctrl set %s, %d out of range\n", v4l2_ctrl_get_name(ctrl->id), ctrl->val);
		return 0;
...
static int camera_module_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	DBG_INFO("");
	if (enable && (ACTS_ISP_PREVIEW_MODE == priv->pcv_mode)) {
		DBG_INFO("ACTS_ISP_PREVIEW_MODE....\n");
		enter_preview_mode(client->adapter);
	}
	if (enable && (ACTS_ISP_CAPTURE_MODE == priv->pcv_mode)) {
		DBG_INFO("ACTS_ISP_CAPTURE_MODE....\n");
...
static void camera_module_init_ops(struct v4l2_ctrl_handler *hdl, const struct v4l2_ctrl_ops *ops)
{
	...
	for (i = 0; i < cmd_array_size; i++) {
		...
		if (NULL == ret) {
			DBG_ERR("ctr[%d] - id:%d, min:%d, max:%d, step:%d, def:%d", i,
...
static void camera_module_init_worker(struct work_struct *work)
{
	int ret = 0;

	sensor_power_on(g_sensor_cfg.rear, &g__spinfo, true);
	ret = module_soft_reset(camera_client);
	if (0 > ret)
		return;
	ret = camera_write_array(camera_client->adapter, module_init_regs);
	if (0 > ret)
		return;

	/* change i2c address from 0x1a to 0x3c for GS5604  */
	update_after_init(camera_client->adapter);
#ifdef CAMERA_MODULE_WITH_MOTOR
	motor_init(camera_client);
	if (0 > ret)
		return;
#endif

#ifdef MODULE_SUPPORT_AF
	ret = camera_write_array(camera_client->adapter,
		module_init_auto_focus);
	if (0 > ret)
		return;
	msleep(10);
#endif
	...
	const struct i2c_device_id *did)
{
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		DBG_ERR("[camera] I2C-Adapter doesn't support I2C_FUNC_SMBUS_BYTE_DATA\n");
		return -EIO;
	}
	...
	if (priv->hdl.error) {
		ret = priv->hdl.error;
		kfree(priv);
		DBG_ERR("[camera]%s %d, module init error!\n", __FUNCTION__, __LINE__);
		return ret;
	}
	...
	init_wq = create_singlethread_workqueue("camera_init_wq");
	INIT_WORK(&init_work, camera_module_init_worker);
...
static int sensor_mod_init(struct soc_camera_link *link,
	struct platform_device *pdev, struct i2c_driver *idrv)
{
	...
	if (ret) {
		pr_err("fail to register platform\n");
		goto regdev_err;
	}
...
static int __init camera_module_init(void)
{
	...
	if (fornt_sensor_detected && rear_sensor_detected)
		return ret;
	ret = sensor_mod_init(&camera_module_link,
		&asoc_camera_device, &camera_i2c_driver);
	return ret;
}
