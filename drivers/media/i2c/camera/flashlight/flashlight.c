...
#define FLASH_NAME_FLASHLIGHT	"flashlight_flash"
...
static int gpio_pin;
static bool flash_act_level = 1;
static int flash_pin;
static bool flashlight_exist;
...
int flashlight_control(int mode)
{
	int ret = 0;
	unsigned long flags;
	pr_info("[flashlight] function:%s , mode:%d\n", __func__, mode);
	...
	case FLASHLIGHT_FLASH:
		gpio_direction_output(flash_pin, flash_act_level);
		break;
	case FLASHLIGHT_OFF:
		gpio_direction_output(flash_pin, flash_act_level ^ 0x1);
		break;
	case TORCHLIGHT_OFF:
...
static int flashlight_init_gpio(struct flashlight_platform_data *fl_pdata,
				struct platform_device *pdev)
{
	int ret;
	enum of_gpio_flags flags;

	pr_info("[flashlight] function:%s\n", __func__);
	...
	gpio_pin = of_get_named_gpio_flags(pdev->dev.of_node,
					   "flashlight-enable", 0, &flags);
	pr_info("flashlight GPIO: %d, flags = %d\n", gpio_pin, flags);
	gpio_act_level = flags & OF_GPIO_ACTIVE_LOW;
	pr_info("gpio_act_level = %d\n", gpio_act_level);
	...
	flash_pin = of_get_named_gpio_flags(pdev->dev.of_node,
					    "flashlight-flash", 0, &flags);
	pr_info("flashlight FLASH_GPIO: %d, flags = %d\n", flash_pin, flags);
	flash_act_level = flags & OF_GPIO_ACTIVE_LOW;
	pr_info("flash_act_level = %d\n", flash_act_level);
	ret = gpio_request(flash_pin, FLASH_NAME_FLASHLIGHT);
	if (ret == 0) {
		gpio_direction_output(gpio_pin, gpio_act_level ^ 0x1);
		flashlight_exist = true;
	} else {
...
static int flashlight_probe(struct platform_device *pdev)
{
	...
	int err = 0;
	pr_info("[flashlight] function:%s ,line:%d\n", __func__, __LINE__);
...
static int __init flashlight_init(void)
{
	...
	pr_info("[flash light] %s version: %s, 2015-01-08\n",
