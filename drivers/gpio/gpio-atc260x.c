...
static inline struct atc260x_gpio_dev *_get_atc260x_gpio_dev(struct gpio_chip
							     *chip)
{
...
static int _atc260x_gpio_request(struct gpio_chip *chip, unsigned offset)
{
	...
	if (hwinfo->mfp_map_tbl == NULL) {	/* need to touch MFP ? */
		...
		if (mfp_reg_msk != 0) {	/* need to touch MFP ? */
			...
			ret =
			    atc260x_reg_setbits(atc260x_gpio->atc260x, mfp_reg,
						mfp_reg_msk, mfp_reg_val);
			if (ret)
				return ret;
		}
	}
...
static int _atc260x_gpio_get_direction(struct gpio_chip *chip, unsigned offset)
{
	...
	ret =
	    atc260x_reg_read(atc260x_gpio->atc260x,
			     hwinfo->reg_outen_tbl[reg_bank]);
	if (ret < 0)
		return ret;
	if (ret & reg_msk)
...
static int _atc260x_gpio_direction_in(struct gpio_chip *chip, unsigned offset)
{
	...
	ret = atc260x_reg_setbits(atc260x_gpio->atc260x,
				  hwinfo->reg_inen_tbl[reg_bank], reg_msk,
				  reg_msk);
	if (ret == 0) {
		ret = atc260x_reg_setbits(atc260x_gpio->atc260x,
					  hwinfo->reg_outen_tbl[reg_bank],
...
static int _atc260x_gpio_direction_out(struct gpio_chip *chip, unsigned offset,
				       int value)
{
	struct atc260x_gpio_dev *atc260x_gpio;
	struct atc260x_gpio_hwinfo const *hwinfo;
...
static void _atc260x_gpio_set(struct gpio_chip *chip, unsigned offset,
			      int value)
{
	struct atc260x_gpio_dev *atc260x_gpio;
	struct atc260x_gpio_hwinfo const *hwinfo;
	...
	ret = atc260x_reg_setbits(atc260x_gpio->atc260x,
				  ...
				  reg_msk, ((value != 0) ? reg_msk : 0));
	if (ret) {
		dev_err(atc260x_gpio->dev,
			"failed to set GPIO%u state to %d, ret=%d\n", offset,
			value, ret);
	}
}
...
{
	static const struct atc260x_gpio_hwinfo *const
	    sc_hwinfo_tbl[ATC260X_ICTYPE_CNT] = {
	...
	atc260x_gpio =
	    devm_kzalloc(&pdev->dev, sizeof(*atc260x_gpio), GFP_KERNEL);
