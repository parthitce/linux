...
#define ATC2609A_BATTERY_DRV_TIMESTAMP		("20150827173016")
 ...
 *
 * @log_switch :i f 1, sotre cap gauge log to sdcad.
 ...
 * @rsense : battery resistor sensor value, affect battery current calclation.
...
/*if clmt_reset is 1, indicate that clmt need reset after upgrade*/
static int clmt_reset;
module_param(clmt_reset, int, S_IRUSR);
...
extern int atc260x_charger_get_rsense(void);
...
extern int atc260x_get_bat_online_status(void);
	...
	struct bat_dts_items *items)
{
	...
	/*rsense*/
	items->rsense = atc260x_charger_get_rsense();
	if (items->rsense < 0)
...
static int atc2609a_bat_measure_vol(void)
{
	...
	data = atc260x_reg_read(battery->atc260x, ATC2609A_PMU_ADC12B_V);
...
static int measure_current_avr(void)
{
	...
	for (i = 0; i < 10; i++) {
		usleep_range(2000, 2200);
		sum += atc2609a_bat_measure_current();
	}

	return sum / 10;
}

static int atc2609a_bat_check_online(struct atc2609a_battery *battery)
{
	...
	data = atc260x_set_bits(atc260x, ATC2609A_PMU_SWCHG_CTL1,
		PMU_SWCHG_CTL1_EN_BAT_DET, PMU_SWCHG_CTL1_EN_BAT_DET);
	if (data < 0)
		return data;
	...
	if (data & PMU_SWCHG_CTL4_BAT_EXT)
		battery->online = 1;
	else
		battery->online = 0;
...
static  int  atc2609a_clmt_get_remain_clmb(struct atc2609a_clmt *clmt)
{
	...
	data = atc260x_reg_read(clmt->atc260x, ATC2609A_CLMT_DATA2);
...
static int  atc2609a_clmt_get_avail_clmb(struct atc2609a_clmt *clmt)
{
	...
	data = atc260x_reg_read(clmt->atc260x, ATC2609A_CLMT_DATA3);
	if (items->rsense == RSENSE_10mohm)
		data =  (data * ADC_LSB_FOR_10mohm + CONST_ROUNDING) /
			CONST_FACTOR;
	else if (items->rsense == RSENSE_20mohm)
...
static int atc2609a_clmt_get_fcc(struct atc2609a_clmt *clmt)
{
	...
	fcc =  atc260x_reg_read(clmt->atc260x, ATC2609A_CLMT_DATA0);
	...
	int *status)
{
	...
	*status = atc2609a_bat_get_charging(measure_current_avr());
...
/**
 * atc2609a_rtc_acce_en - rtc accelerate enable.
 * if enable, then rtc's clk will up to 250 times than normal state.
 */
#ifdef CLMT_RESET_ENABLE
static void atc2609a_rtc_acce_en(struct atc260x_dev *atc260x,
		bool enable)
{
	if (enable)
		atc260x_set_bits(atc260x, ATC2609A_RTC_CTL,
			PMU_RTC_CTL_VERI, PMU_RTC_CTL_VERI);
	else
		atc260x_set_bits(atc260x, ATC2609A_RTC_CTL,
			PMU_RTC_CTL_VERI, 0);
...
static void atc2609a_bat_dump(struct atc2609a_battery *battery)
{
	...
	pr_info("[%s] VOL:%d\n", __func__,
		atc2609a_bat_measure_vol());
	pr_info("[%s] OCV:%d\n", __func__,
		battery->bat_ocv);
	pr_info("[%s] R(CH):%d\n", __func__,
		battery->bat_ch_r);
	pr_info("[%s] CUR:%d\n", __func__,
		atc2609a_bat_measure_current());
	pr_info("[%s] CLMBd:%d\n",
		__func__,
...
static void atc2609a_bat_post_process(struct atc2609a_battery *battery)
{
	...
	atc260x_pstore_set(battery->atc260x,
		ATC260X_PSTORE_TAG_GAUGE_CAP, clmt->soc_show);
...
static int atc2609a_bat_pm_notify(struct notifier_block *nb,
...
static struct notifier_block atc2609a_bat_pm_notifier = {
	.notifier_call = atc2609a_bat_pm_notify,
};
...
static void atc2609a_bat_init_work(struct work_struct *work)
{
	...
	atc2609a_bat_check_online(battery);
...
static  int atc2609a_bat_probe(struct platform_device *pdev)
{
	...
	queue_delayed_work(battery->wq, &battery->init_work, 0 * HZ);
...
static int atc2609a_bat_suspend(struct device *dev)
{
	...
	clmt->suspend_delta_clmb =
		atc2609a_clmt_get_delta_clmb(clmt);
	pr_info("[%s], suspend_time_ms(%ldms),delta_clmb(%dmAh)\n",
		__func__, suspend_time_ms, clmt->suspend_delta_clmb);
...
static void atc2609a_bat_pm_complete(struct device *dev)
{
	struct atc2609a_battery *battery = dev_get_drvdata(dev);
	struct atc2609a_clmt *clmt = battery->clmt;

	/*
		* If suspend called fail,  suspend_time_ms will be zero.
		*/
	if (suspend_time_ms <= 0)
		return;

	suspend_interval_ms = get_sys_tick_ms() -
		suspend_time_ms;
	pr_info("[%s], suspend_interval_ms=resume_time_ms-suspend_time_ms="
		"%ld-%ld=%ld\n",
		__func__,
		get_sys_tick_ms(),
		suspend_time_ms,
...
static const struct dev_pm_ops atc2609a_bat_pm_ops = {
	...
	.complete      = atc2609a_bat_pm_complete,
};
