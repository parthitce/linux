...
static enum CONSTANT_CURRENT
	atc2609a_charger_cc2regval(enum CHARGER_MODE mode,
		int data)
{
	int value = 0;
...
static int  atc2609a_charger_set_rsense(struct atc260x_dev *atc260x, int rsense)
{
	if (rsense == 10) {
		atc260x_set_bits(atc260x, ATC2609A_PMU_SWCHG_CTL0,
			SWCHG_CTL0_RSENSEL_MASK,
			SWCHG_CTL0_RSENSEL_10mohm);
	} else if (rsense == 20) {
		atc260x_set_bits(atc260x, ATC2609A_PMU_SWCHG_CTL0,
			SWCHG_CTL0_RSENSEL_MASK,
			SWCHG_CTL0_RSENSEL_20mohm);
	} else {
...
static int  atc2609a_charger_get_rsense(struct atc260x_dev *atc260x)
{
	int data;

	data = atc260x_reg_read(atc260x, ATC2609A_PMU_SWCHG_CTL0);
	if (data & SWCHG_CTL0_RSENSEL_MASK)
		return 10;
	else
		return 20;
...
static void atc2609a_charger_init_base(struct atc260x_dev *atc260x)
{
	...
	atc2609a_charger_set_rsense(atc260x, items.rsense);
	/* wall init*/
...
struct atc260x_charger_ops atc2609a_charger_ops = {
	...
	.get_rsense = atc2609a_charger_get_rsense,
};
