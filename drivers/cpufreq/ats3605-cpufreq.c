...
/* cpu temp clock */
#define CPU_TMP_CLK_NAME	"vce"



#define CPUFREQ_LOWER_BURN_MAX_FREQ		600000

#define CPU_REGULATOR_NAME	"dcdc1"
#define AVDD_REGULATOR_NAME	"ldo6"
...
static int cpu0_cpufreq_driver_init(void)
{
	...
	avdd_reg = devm_regulator_get(cpu_dev, AVDD_REGULATOR_NAME);
	if (IS_ERR(avdd_reg)) {
		CPUFREQ_ERR("failed to get avdd  regulator\n");
		avdd_reg = NULL;
	}
	...
	if (cpu_reg) {
		...
		if (avdd_reg)
			regulator_set_voltage(avdd_reg, 1200000, 1200000);
