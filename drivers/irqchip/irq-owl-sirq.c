...
static int owl_sirq_set_type(struct irq_data *data, unsigned int flow_type)
{
	...
	ret = irq_chip_set_type_parent(data, IRQ_TYPE_LEVEL_HIGH);
	raw_spin_unlock_irqrestore(&chip_data->lock, flags);
	pr_info("%s called success %d\n", __func__, ret);
				     ...
				     unsigned int *type)
{
	...
	if (is_of_node(fwspec->fwnode)) {
		...
		//*hwirq = fwspec->param[0] + INTC_GIC_INTERRUPT_PIN;
		*hwirq = fwspec->param[0];
		*type = fwspec->param[1] & IRQ_TYPE_SENSE_MASK;
		pr_info("%s called hwirq %ld type %d\n", __func__, *hwirq, *type);
		return 0;
	}
				 ...
				 unsigned int nr_irqs, void *arg)
{
	...
	struct irq_fwspec parent_fwspec = {
		...
		.param[2]	= IRQ_TYPE_LEVEL_HIGH,
		.fwnode		= domain->parent->fwnode,
	};

	pr_info("%s called nr_irqs %d\n", __func__, nr_irqs);
	if (WARN_ON(nr_irqs != 1))
		return -EINVAL;

	int type, i;
	unsigned long hwirq;
	owl_sirq_domain_translate(domain, fwspec, &hwirq, &type);

	for (i = 0; i < nr_irqs; i++)
		irq_domain_set_hwirq_and_chip(domain, virq, hwirq + i,
				&owl_sirq_chip,
				domain->host_data);
					...
					struct device_node *parent)
{
	...
	chip_data->sirq = kcalloc(sirq_cnt, sizeof(*chip_data->sirq), GFP_KERNEL);
	if (!chip_data->sirq)
		goto out_free;
	...
	chip_data->shared_reg = of_property_read_bool(node, "actions,sirq-shared-reg");
	for (i = 0; i < sirq_cnt; i++) {
		u32 value;

		ret = of_property_read_u32_index(node, "actions,sirq-offset", i, &value);
		if (ret)
			goto out_unmap;

		get_sirq_offset(i) = (u16)value;

		ret = of_property_read_u32_index(node, "actions,sirq-clk-sel", i, &value);
		if (ret || !value)
			continue;

		/* external interrupt controller can be either connect to 32Khz or
		 * 24Mhz external/internal clock. This needs to be configured for
