...
static int ksz8081_probe(struct phy_device *phydev)
{
	int reg_val;
	int cnt = 0;
	...
	/*ksz8081 phy ctrl2*/
	reg_val = phy_read(phydev, MII_KSZPHY_CTRL);
	phy_reg_set_bits(phydev, MII_KSZPHY_CTRL, 0x80);
	return 0;
}

static int ksz8081_suspend(struct phy_device *phydev)
{
	return 0;
}
