...
#define WIFI_MAC_NAME				"WIFIMAC"	
#define	WIFI_MAC_MISC_LEN			32
	...
	char * buf)
{
	...
	pHalData->efuse_file_status = _FALSE;
...
#ifdef CONFIG_EFUSE_CONFIG_FILE
#ifdef CONFIG_PLATFORM_OWL
	memset(buf, 0, WIFI_MAC_MISC_LEN);
	printk("ReadEfuseInfo8189FS ---> read_mi_item:%s\n", WIFI_MAC_NAME);
	ret = read_mi_item(WIFI_MAC_NAME, buf, WIFI_MAC_MISC_LEN);
	if (ret >= 0){
		Hal_ReadMACAddrFromMiscinfo_8188ES(padapter, buf);
	}else {
		DBG_871X("not get_wifimac_form_miscinfo!!\n");
		DBG_871X("read wifimac from file!!\n");
		Hal_ReadMACAddrFromFile(padapter, pHalData->EEPROMMACAddr);
	}
#else
	DBG_871X("wifimac does not exist in misinfo!!\n");
    DBG_871X("read wifimac from file!!\n");
    Hal_ReadMACAddrFromFile(padapter, pHalData->EEPROMMACAddr);
#endif //CONFIG_PLATFORM_OWL
