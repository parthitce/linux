...
static int rtw_suspend(struct usb_interface *pusb_intf, pm_message_t message)
{
	...
	DBG_871X_LEVEL(_drv_always_, "==>rtw_suspend\n");
	...
	DBG_871X_LEVEL(_drv_always_, "rtw_suspend -> rtw_suspend_common\n");

	ret =  rtw_suspend_common(padapter);

exit:
	
	DBG_871X_LEVEL(_drv_always_, "<==rtw_suspend,.......... ret:%d\n", ret);
	return ret;
}
...
static int rtw_resume(struct usb_interface *pusb_intf)
{
	...
	DBG_871X_LEVEL(_drv_always_, "==>rtw_resume\n");
...
#ifdef CONFIG_WOWLAN
	DBG_871X_LEVEL(_drv_always_, "!!!define CONFIG_WOWLAN!!!\n");
...
static int __init rtw_drv_entry(void)
{
	int ret = 0;
#ifdef CONFIG_PLATFORM_OWL
	unsigned int usb_port_number = 1;
	struct device_node *of_node;

	of_node = of_find_compatible_node(NULL, NULL, "wifi,bt,power,ctl");
	if (of_node != NULL) {
		if (of_find_property(of_node, "wifi_use_usb_port_number",
			NULL)) {
			ret = of_property_read_u32(of_node,
				 "wifi_use_usb_port_number", &usb_port_number);
			if (!ret) {
				pr_err("wifi use usb port number:%d\n",
					usb_port_number);
				aotg_id = usb_port_number;
			}
	    } else {
			pr_err("No \"wifi_use_usb_port_number\" property found in dts\n");
		}
