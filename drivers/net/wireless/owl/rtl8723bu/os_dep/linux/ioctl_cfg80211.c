...
if (padapter->registrypriv.mp_mode == 1)
{
	...
	if (pmlmepriv->LinkDetectInfo.bBusyTraffic == _TRUE)
	{
		...
		((passtime > 8000) && is_miracast_enabled(padapter->wfd_info.stack_wfd_mode)) ||
#ifdef CONFIG_CONCURRENT_MODE
		((passtime > 8000) && padapter->pbuddy_adapter && is_miracast_enabled(padapter->pbuddy_adapter->wfd_info.stack_wfd_mode))	
#endif
		)
#else	
		if (passtime > BUSY_TRAFFIC_SCAN_DENY_PERIOD)
