...
#ifdef CUSTOMER_OOB
static uint wifi_wakeup_host_gpios = 0;
	...
	if (NULL == np) {
		...
		if (of_find_property(np, "wifi_wakeup_host_gpios", NULL)) {
			...
			if (gpio_is_valid(wifi_wakeup_host_gpios)
				&& (gpio_request(wifi_wakeup_host_gpios, "wifi_wakeup_host_gpios") >= 0)) {
				...
				printk("%s gpio for wifi_wakeup_host_gpios invalid or can't request\n", __FUNCTION__);
			}
		}
	}
/*
...
{
	if (gpio_is_valid(wifi_wakeup_host_gpios))
		gpio_free(wifi_wakeup_host_gpios);
}
