...
int dhd_ioctl_entry_by_kernel(struct net_device *net, struct ifreq *ifr, int cmd)
{
	dhd_info_t *dhd = DHD_DEV_INFO(net);
	dhd_ioctl_t ioc;
	int bcmerror = 0;
...
/* call in kernel, no need compat transfor  */
//#ifdef CONFIG_COMPAT
#if 0
	if (is_compat_task()) {
		compat_wl_ioctl_t compat_ioc;
...
#ifdef CONFIG_PLATFORM_OWL
	err = owl_wifi_init(WIFI_TYPE_BCMDHD, &dhd_module_shutdown);
	if (unlikely(err < 0)) {
