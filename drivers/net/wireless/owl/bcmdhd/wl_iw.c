...
extern int dhd_ioctl_entry_by_kernel(struct net_device *net, struct ifreq *ifr, int cmd);
...
)
{
	...
	if (0 == memcmp(ifr.ifr_name, iface_name, strlen(iface_name))) {
		/* wlan interface support SIOCDEVPRIVATE  */
		printk("kernel call ioctl,cmd:%d\n", cmd);
		ret = dhd_ioctl_entry_by_kernel(dev, &ifr, SIOCDEVPRIVATE);
	} else {
		/* p2p ioctl, not support SIOCDEVPRIVATE, just return (-1)  */
