...
extern void owl_gmac_suspend(struct device *device);
extern void owl_gmac_resume(struct device *device);
...
bool stmmac_eee_init(struct stmmac_priv *priv)
{
	...
	if (priv->dma_cap.eee) {
		...
		//pr_info("stmmac: Energy-Efficient Ethernet initialized:%d\n",priv->pcs);
...
static void stmmac_check_pcs_mode(struct stmmac_priv *priv)
{
	...
	pr_info("stmmac_check_pcs_mode:%x-%x\n", interface, priv->dma_cap.pcs);
	if (priv->dma_cap.pcs) {
...
static int stmmac_init_phy(struct net_device *dev)
{
	...
	snprintf(phy_id_fmt, MII_BUS_ID_SIZE + 3, PHY_ID_FMT, bus_id,
		 priv->plat->phy_addr);
	pr_info("stmmac_init_phy:  trying to attach to %s-%d-%d\n", phy_id_fmt,
		interface, priv->plat->phy_addr);
...
int stmmac_resume(struct net_device *ndev)
{
	...
	pr_info("stmmac_resume\n");
	owl_gmac_resume(priv->device);
	if (netif_running(ndev))
		stmmac_open(ndev);
