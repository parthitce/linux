...
static uint32_t pwm_ctl_reg_array[] = {
	PWM_CTL0, PWM_CTL1, PWM_CTL2, PWM_CTL3,
};
			  ...
			  int duty_ns, int period_ns)
{
	...
	if (period_ns != channel->required_period_ns) {
		...
		channel->period_ns = NS_IN_HZ / clk_get_rate(channel->clk) * 64;
	}

	/* round up or down */
	val = ((((duty_ns) << 6) << 1) / (period_ns) + 1) >> 1;
	channel->duty_ns = ((channel->period_ns * val) >> 6);
...
static struct owl_pwm_chip_data pwm_chip_data[] = {
	{ .chip_type = 1, .pwm_num = 6 }, /* s500 & s700 */
	{ .chip_type = 2, .pwm_num = 6 }, /* s900 */
};
