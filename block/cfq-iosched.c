...
void boost_cma_requests(struct block_device *bd, void *arg)
{
	...
	/* do not use this optimization in situation when uhost disk's random
	write speed too low, in that case , cma boost will break continuous write
	into many small random writes. */
	if (MAJOR(bd->bd_dev) == SCSI_DISK0_MAJOR)
		return;
